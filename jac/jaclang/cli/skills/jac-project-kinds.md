...
| Microservices | Same code split into services via `[scale.microservices.routes]` in jac.toml (imports of cut modules become HTTP RPCs; consumer auto-starts providers) | `jac scale split <module>`; `jac run --port N consumer.jac`; `JAC_SV_<MOD>_URL` to split hosts | `jac-sv-microservices`, `jac-sv-endpoints`, `jac-sv-deploy` |
...
| Desktop app | The full-stack app wrapped in one nacompiled binary embedding the OS webview | `jac run --client desktop` / `jac build --client desktop` | `jac-desktop-app`, `jac-fullstack-patterns` |
