...
from jaclang.lib.jaclib import destroy
import math
import warnings
...
class MultiScaleDeformableAttention(nn.Module):
    ...
    def forward(self: Any, value: Tensor, value_spatial_shapes: Tensor, value_spatial_shapes_list: List[Tuple], level_start_index: Tensor, sampling_locations: Tensor, attention_weights: Tensor, im2col_step: int) -> object:
...
class GroundingDinoFrozenBatchNorm2d(nn.Module):
    ...
    def __init__(self: Any, n: Any) -> object:
        super().__init__()
    ...
    def _load_from_state_dict(self: Any, state_dict: Any, prefix: Any, local_metadata: Any, strict: Any, missing_keys: Any, unexpected_keys: Any, error_msgs: Any) -> object:
        num_batches_tracked_key = prefix + 'num_batches_tracked'
        if num_batches_tracked_key in state_dict:
            destroy([state_dict[num_batches_tracked_key]])
...
def replace_batch_norm(model: Any) -> object:
    '''r"""
...
class GroundingDinoConvEncoder(nn.Module):
    ...
    def forward(self: Any, pixel_values: torch.Tensor, pixel_mask: torch.Tensor) -> object:
...
class GroundingDinoConvModel(nn.Module):
    ...
    def __init__(self: Any, conv_encoder: Any, position_embedding: Any) -> object:
        super().__init__()
        self.conv_encoder = conv_encoder
        self.position_embedding = position_embedding

    def forward(self: Any, pixel_values: Any, pixel_mask: Any) -> object:
...
class GroundingDinoLearnedPositionEmbedding(nn.Module):
    ...
    def forward(self: Any, pixel_values: Any, pixel_mask: Any=None) -> object:
...
def build_position_encoding(config: Any) -> object:
...
class GroundingDinoMultiscaleDeformableAttention(nn.Module):
    ...
    def __init__(self: Any, config: GroundingDinoConfig, num_heads: int, n_points: int) -> object:
        super().__init__()
        ...
        if not (dim_per_head & dim_per_head - 1 == 0 and dim_per_head != 0):
            warnings.warn('You\'d better set embed_dim (d_model) in GroundingDinoMultiscaleDeformableAttention to make the"\n                " dimension of each attention head a power of 2 which is more efficient in the authors\' CUDA"\n                " implementation.')
        self.im2col_step = 64
    ...
    def with_pos_embed(self: Any, tensor: torch.Tensor, position_embeddings: Optional[Tensor]) -> object:
        return tensor if position_embeddings is None else tensor + position_embeddings

    def forward(self: Any, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor]=None, encoder_hidden_states: Any=None, encoder_attention_mask: Any=None, position_embeddings: Optional[torch.Tensor]=None, reference_points: Any=None, spatial_shapes: Any=None, spatial_shapes_list: Any=None, level_start_index: Any=None, output_attentions: bool=False) -> object:
        if position_embeddings is not None:
            hidden_states = self.with_pos_embed(hidden_states, position_embeddings)
        batch_size, num_queries, _ = hidden_states.shape
        batch_size, sequence_length, _ = encoder_hidden_states.shape
        torch._assert_async(torch.logical_not((spatial_shapes[:, 0] * spatial_shapes[:, 1]).sum() != sequence_length), '[[GM-TRAP ValueError]]Make sure to align the spatial shapes with the sequence length of the encoder hidden states[[/GM-TRAP]]')
...
class GroundingDinoTextEnhancerLayer(nn.Module):
    ...
    def with_pos_embed(self: Any, hidden_state: Tensor, position_embeddings: Optional[Tensor]) -> object:
        return hidden_state if position_embeddings is None else hidden_state + position_embeddings

    def forward(self: Any, hidden_states: torch.FloatTensor, attention_masks: Optional[torch.BoolTensor]=None, position_embeddings: Optional[torch.FloatTensor]=None) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
...
class GroundingDinoBiMultiHeadAttention(nn.Module):
    ...
    def _reshape(self: Any, tensor: torch.Tensor, seq_len: int, batch_size: int) -> object:
        return tensor.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2).contiguous()

    def forward(self: Any, vision_features: torch.FloatTensor, text_features: torch.FloatTensor, vision_attention_mask: Optional[torch.BoolTensor]=None, text_attention_mask: Optional[torch.BoolTensor]=None) -> Tuple[Tuple[torch.FloatTensor, torch.FloatTensor], Tuple[torch.FloatTensor, torch.FloatTensor]]:
...
class GroundingDinoDropPath(nn.Module):
    ...
    def __init__(self: Any, drop_prob: Optional[float]=None) -> None:
        super().__init__()
...
class GroundingDinoFusionLayer(nn.Module):
    ...
    def forward(self: Any, vision_features: torch.FloatTensor, text_features: torch.FloatTensor, attention_mask_vision: Optional[torch.BoolTensor]=None, attention_mask_text: Optional[torch.BoolTensor]=None) -> Tuple[Tuple[torch.FloatTensor, torch.FloatTensor], Tuple[torch.FloatTensor, torch.FloatTensor]]:
...
class GroundingDinoDeformableLayer(nn.Module):
    ...
    def __init__(self: Any, config: GroundingDinoConfig) -> object:
        super().__init__()
    ...
    def forward(self: Any, hidden_states: torch.Tensor, attention_mask: torch.Tensor, position_embeddings: Optional[torch.Tensor]=None, reference_points: Any=None, spatial_shapes: Any=None, spatial_shapes_list: Any=None, level_start_index: Any=None, output_attentions: bool=False) -> object:
        """
        Args:
            hidden_states (`torch.FloatTensor` of shape `(batch_size, sequence_length, hidden_size)`):
                Input to the layer.
...
def get_sine_pos_embed(pos_tensor: torch.Tensor, num_pos_feats: int=128, temperature: int=10000, exchange_xy: bool=True) -> Tensor:
    ...
    def sine_func(x: torch.Tensor) -> object:
        sin_x = x * scale / dim_t
...
class GroundingDinoEncoderLayer(nn.Module):
    ...
    def get_text_position_embeddings(self: Any, text_features: Tensor, text_position_embedding: Optional[torch.Tensor], text_position_ids: Optional[torch.Tensor]) -> Tensor:
...
class GroundingDinoMultiheadAttention(nn.Module):
    ...
    def __init__(self: Any, config: Any, num_attention_heads: Any=None) -> object:
        super().__init__()
    ...
    def forward(self: Any, queries: torch.Tensor, keys: torch.Tensor, values: torch.Tensor, attention_mask: Optional[torch.FloatTensor]=None, output_attentions: Optional[bool]=False) -> Tuple[torch.Tensor]:
...
class GroundingDinoContrastiveEmbedding(nn.Module):
    ...
    def forward(self: Any, vision_hidden_state: torch.FloatTensor, text_hidden_state: torch.FloatTensor, text_token_mask: torch.BoolTensor) -> torch.FloatTensor:
...
class GroundingDinoEncoder(GroundingDinoPreTrainedModel):
    ...
    @staticmethod
    def get_reference_points(spatial_shapes: Any, valid_ratios: Any, device: Any) -> object:
        """
    ...
    def forward(self: Any, vision_features: Tensor, vision_attention_mask: Tensor, vision_position_embedding: Tensor, spatial_shapes: Tensor, spatial_shapes_list: List[Tuple[int, int]], level_start_index: Tensor, valid_ratios: Any=None, text_features: Optional[Tensor]=None, text_attention_mask: Optional[Tensor]=None, text_position_embedding: Optional[Tensor]=None, text_self_attention_masks: Optional[Tensor]=None, text_position_ids: Optional[Tensor]=None, output_attentions: Any=None, output_hidden_states: Any=None, return_dict: Any=None) -> object:
        '''r"""
        Args:
...
class GroundingDinoDecoder(GroundingDinoPreTrainedModel):
    ...
    def forward(self: Any, inputs_embeds: Any, vision_encoder_hidden_states: Any, vision_encoder_attention_mask: Any=None, text_encoder_hidden_states: Any=None, text_encoder_attention_mask: Any=None, reference_points: Any=None, spatial_shapes: Any=None, spatial_shapes_list: Any=None, level_start_index: Any=None, valid_ratios: Any=None, self_attn_mask: Any=None, output_attentions: Any=None, output_hidden_states: Any=None, return_dict: Any=None) -> object:
        '''r"""
        Args:
...
class GroundingDinoModel(GroundingDinoPreTrainedModel):
    ...
    def freeze_backbone(self: Any) -> object:
        for name, param in self.backbone.conv_encoder.model.named_parameters():
            param.requires_grad_(False)

    def unfreeze_backbone(self: Any) -> object:
        for name, param in self.backbone.conv_encoder.model.named_parameters():
            param.requires_grad_(True)

    def get_valid_ratio(self: Any, mask: Any) -> object:
    ...
    def generate_encoder_output_proposals(self: Any, enc_output: Any, padding_mask: Any, spatial_shapes: Any) -> object:
    ...
    @auto_docstring
    def forward(self: Any, pixel_values: Tensor, input_ids: Tensor, token_type_ids: Optional[Tensor]=None, attention_mask: Optional[Tensor]=None, pixel_mask: Optional[Tensor]=None, encoder_outputs: Any=None, output_attentions: Any=None, output_hidden_states: Any=None, return_dict: Any=None) -> object:
        '''r"""
...
class GroundingDinoMLPPredictionHead(nn.Module):
    ...
    def __init__(self: Any, input_dim: Any, hidden_dim: Any, output_dim: Any, num_layers: Any) -> object:
        super().__init__()
...
def build_text_mask(logits: Any, attention_mask: Any) -> object:
    """
...
class GroundingDinoForObjectDetection(GroundingDinoPreTrainedModel):
    ...
    def forward(self: Any, pixel_values: torch.FloatTensor, input_ids: torch.LongTensor, token_type_ids: Optional[torch.LongTensor]=None, attention_mask: Optional[torch.LongTensor]=None, pixel_mask: Optional[torch.BoolTensor]=None, encoder_outputs: Optional[Union[GroundingDinoEncoderOutput, Tuple]]=None, output_attentions: Optional[bool]=None, output_hidden_states: Optional[bool]=None, return_dict: Optional[bool]=None, labels: Optional[List[Dict[str, Union[torch.LongTensor, torch.FloatTensor]]]]=None) -> object:
        ...
        if labels is not None:
            ...
            loss, loss_dict, auxiliary_outputs = self.loss_function(logits, labels, self.device, pred_boxes, self.config, label_maps, text_mask, outputs_class=outputs_class, outputs_coord=outputs_coord, encoder_logits=outputs[-2], encoder_pred_boxes=outputs[-1])
        if not return_dict:
