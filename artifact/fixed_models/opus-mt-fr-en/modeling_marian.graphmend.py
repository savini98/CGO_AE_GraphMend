...
class MarianSinusoidalPositionalEmbedding(nn.Embedding):
    ...
    def __init__(self: Any, num_positions: int, embedding_dim: int, padding_idx: Optional[int]=None) -> None:
        super().__init__(num_positions, embedding_dim)

    def _init_weight(self: Any) -> object:
        """
    ...
    @torch.no_grad()
    def forward(self: Any, input_ids_shape: torch.Size, past_key_values_length: int=0, position_ids: Optional[torch.Tensor]=None) -> torch.Tensor:
...
class MarianAttention(nn.Module):
    ...
    def __init__(self: Any, embed_dim: int, num_heads: int, dropout: float=0.0, is_decoder: bool=False, bias: bool=True, is_causal: bool=False, config: Optional[MarianConfig]=None, layer_idx: Optional[int]=None) -> object:
        super().__init__()
...
class MarianEncoderLayer(nn.Module):
    ...
    def __init__(self: Any, config: MarianConfig, layer_idx: Optional[int]=None) -> object:
        super().__init__()
        self.embed_dim = config.d_model
        self.self_attn = MARIAN_ATTENTION_CLASSES[config._attn_implementation](embed_dim=self.embed_dim, num_heads=config.encoder_attention_heads, dropout=config.attention_dropout, config=config, layer_idx=layer_idx)
...
class MarianDecoderLayer(nn.Module):
    ...
    def __init__(self: Any, config: MarianConfig, layer_idx: Optional[int]=None) -> object:
        super().__init__()
        self.embed_dim = config.d_model
        self.self_attn = MARIAN_ATTENTION_CLASSES[config._attn_implementation](embed_dim=self.embed_dim, num_heads=config.decoder_attention_heads, dropout=config.attention_dropout, is_decoder=True, is_causal=True, config=config, layer_idx=layer_idx)
        self.dropout = config.dropout
        self.activation_fn = ACT2FN[config.activation_function]
        self.activation_dropout = config.activation_dropout
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = MARIAN_ATTENTION_CLASSES[config._attn_implementation](self.embed_dim, config.decoder_attention_heads, dropout=config.attention_dropout, is_decoder=True, config=config, layer_idx=layer_idx)
...
class MarianPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self: Any, module: Union[nn.Linear, nn.Embedding, MarianSinusoidalPositionalEmbedding]) -> object:
...
class MarianEncoder(MarianPreTrainedModel):
    ...
    def __init__(self: Any, config: MarianConfig, embed_tokens: Optional[nn.Embedding]=None) -> object:
    ...
    def forward(self: Any, input_ids: Optional[torch.LongTensor]=None, attention_mask: Optional[torch.LongTensor]=None, head_mask: Optional[torch.Tensor]=None, inputs_embeds: Optional[torch.FloatTensor]=None, output_attentions: Optional[bool]=None, output_hidden_states: Optional[bool]=None, return_dict: Optional[bool]=None) -> Union[Tuple[torch.Tensor], BaseModelOutput]:
        '''r"""
        Args:
...
class MarianDecoder(MarianPreTrainedModel):
    ...
    def forward(self: Any, input_ids: Optional[torch.LongTensor]=None, attention_mask: Optional[torch.Tensor]=None, encoder_hidden_states: Optional[torch.FloatTensor]=None, encoder_attention_mask: Optional[torch.LongTensor]=None, head_mask: Optional[torch.Tensor]=None, cross_attn_head_mask: Optional[torch.Tensor]=None, past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]]=None, inputs_embeds: Optional[torch.FloatTensor]=None, use_cache: Optional[bool]=None, output_attentions: Optional[bool]=None, output_hidden_states: Optional[bool]=None, return_dict: Optional[bool]=None, cache_position: Optional[torch.Tensor]=None) -> Union[Tuple[torch.Tensor], BaseModelOutputWithPastAndCrossAttentions]:
        ...
        if use_cache and (not isinstance(past_key_values, Cache)):
            ...
            __jac_log_emit__(_gm_log_slot_1, ('Passing a tuple of `past_key_values` is deprecated and will be removed in Transformers v4.58.0. "\n                "You should pass an instance of `EncoderDecoderCache` instead, e.g. "\n                "`past_key_values=EncoderDecoderCache.from_legacy_cache(past_key_values)`.',), {})
            past_key_values = EncoderDecoderCache.from_legacy_cache(past_key_values)
        ...
        for attn_mask, mask_name in zip([head_mask, cross_attn_head_mask], ['head_mask', 'cross_attn_head_mask']):
            if attn_mask is not None:
                assert attn_mask.size()[0] == len(self.layers), f'The `{mask_name}` should be specified for {len(self.layers)} layers, but it is for"\n                    f" {head_mask.size()[0]}.'
...
class MarianModel(MarianPreTrainedModel):
    ...
    def __init__(self: Any, config: MarianConfig) -> object:
        super().__init__(config)
    ...
    def get_decoder_input_embeddings(self: Any) -> object:
        if self.config.share_encoder_decoder_embeddings:
            raise ValueError('`get_decoder_input_embeddings` should not be called if `config.share_encoder_decoder_embeddings` "\n                "is `True`. Please use `get_input_embeddings` instead.')
        return self.get_decoder().get_input_embeddings()

    def set_decoder_input_embeddings(self: Any, value: Any) -> object:
        if self.config.share_encoder_decoder_embeddings:
            raise ValueError('`config.share_encoder_decoder_embeddings` is set to `True` meaning the decoder input embeddings "\n                "are shared with the encoder. In order to set the decoder input embeddings, you should simply set "\n                "the encoder input embeddings by calling `set_input_embeddings` with the appropriate embeddings.')
    ...
    def resize_decoder_token_embeddings(self: Any, new_num_tokens: int) -> nn.Embedding:
        if self.config.share_encoder_decoder_embeddings:
            raise ValueError('`resize_decoder_token_embeddings` should not be called if `config.share_encoder_decoder_embeddings` "\n                "is `True`. Please use `resize_token_embeddings` instead.')
...
class MarianMTModel(MarianPreTrainedModel, GenerationMixin):
    ...
    def _resize_token_embeddings(self: Any, new_num_tokens: int, pad_to_multiple_of: Any=None, *args: Any) -> nn.Embedding:
    ...
    def resize_decoder_token_embeddings(self: Any, new_num_tokens: Any) -> object:
        if self.config.share_encoder_decoder_embeddings:
            raise ValueError('`resize_decoder_token_embeddings` should not be called if `config.share_encoder_decoder_embeddings` "\n                "is `True`. Please use `resize_token_embeddings` instead.')
    ...
    def set_output_embeddings(self: Any, new_embeddings: nn.Embedding) -> object:
    ...
    def forward(self: Any, input_ids: Optional[torch.LongTensor]=None, attention_mask: Optional[torch.Tensor]=None, decoder_input_ids: Optional[torch.LongTensor]=None, decoder_attention_mask: Optional[torch.Tensor]=None, head_mask: Optional[torch.Tensor]=None, decoder_head_mask: Optional[torch.Tensor]=None, cross_attn_head_mask: Optional[torch.Tensor]=None, encoder_outputs: Optional[Union[Tuple[torch.Tensor], BaseModelOutput]]=None, past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]]=None, inputs_embeds: Optional[torch.FloatTensor]=None, decoder_inputs_embeds: Optional[torch.FloatTensor]=None, labels: Optional[torch.LongTensor]=None, use_cache: Optional[bool]=None, output_attentions: Optional[bool]=None, output_hidden_states: Optional[bool]=None, return_dict: Optional[bool]=None, cache_position: Optional[torch.Tensor]=None) -> Seq2SeqLMOutput:
        ...
        if labels is not None:
            if use_cache:
                __jac_log_emit__(_gm_log_slot_2, ('The `use_cache` argument is changed to `False` since `labels` is provided.',), {})
            use_cache = False
...
_gm_log_slot_2 = __jac_log_register__(logger.warning)
