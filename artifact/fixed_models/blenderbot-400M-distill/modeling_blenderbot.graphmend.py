...
class BlenderbotAttention(nn.Module):
    ...
    def __init__(self: Any, embed_dim: int, num_heads: int, dropout: float=0.0, is_decoder: bool=False, bias: bool=True, is_causal: bool=False, config: Optional[BlenderbotConfig]=None, layer_idx: Optional[int]=None) -> object:
        super().__init__()
...
class BlenderbotEncoderLayer(nn.Module):
    ...
    def __init__(self: Any, config: BlenderbotConfig) -> object:
        super().__init__()
    ...
    def forward(self: Any, hidden_states: torch.Tensor, attention_mask: torch.Tensor, layer_head_mask: torch.Tensor, output_attentions: bool=False) -> torch.Tensor:
        """
        Args:
...
class BlenderbotDecoderLayer(nn.Module):
    ...
    def __init__(self: Any, config: BlenderbotConfig, layer_idx: Optional[int]=None) -> object:
        super().__init__()
        self.embed_dim = config.d_model
        self.self_attn = BLENDERBOT_ATTENTION_CLASSES[config._attn_implementation](embed_dim=self.embed_dim, num_heads=config.decoder_attention_heads, dropout=config.attention_dropout, is_decoder=True, is_causal=True, config=config, layer_idx=layer_idx)
        self.dropout = config.dropout
        self.activation_fn = ACT2FN[config.activation_function]
        self.activation_dropout = config.activation_dropout
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = BLENDERBOT_ATTENTION_CLASSES[config._attn_implementation](self.embed_dim, config.decoder_attention_heads, dropout=config.attention_dropout, is_decoder=True, config=config, layer_idx=layer_idx)
...
class BlenderbotEncoder(BlenderbotPreTrainedModel):
    ...
    def __init__(self: Any, config: BlenderbotConfig, embed_tokens: Optional[nn.Embedding]=None) -> object:
...
class BlenderbotModel(BlenderbotPreTrainedModel):
    ...
    @classmethod
    def from_pretrained(cls: Any, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args: Any, **kwargs: Any) -> object:
        if pretrained_model_name_or_path == 'facebook/blenderbot-90M':
            warnings.warn('The checkpoint `facebook/blenderbot-90M` is deprecated. In the future, please use the identical"\n                " checkpoint `facebook/small_blenderbot-90M` with"\n                " `BlenderbotSmallModel.from_pretrained(\'facebook/small_blenderbot-90M\')` instead.', FutureWarning)
...
class BlenderbotForConditionalGeneration(BlenderbotPreTrainedModel, GenerationMixin):
    ...
    def from_pretrained(cls: Any, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args: Any, **kwargs: Any) -> object:
        if pretrained_model_name_or_path == 'facebook/blenderbot-90M':
            warnings.warn('The checkpoint `facebook/blenderbot-90M` is deprecated. In the future, please use the identical"\n                " checkpoint `facebook/small_blenderbot-90M` with"\n                " `BlenderbotSmallForConditionalGeneration.from_pretrained(\'facebook/small_blenderbot-90M\')` instead.', FutureWarning)
