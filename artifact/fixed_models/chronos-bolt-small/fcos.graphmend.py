...
class FCOSHead(nn.Module):
    ...
    def __init__(self: Any, in_channels: int, num_anchors: int, num_classes: int, num_convs: Optional[int]=4) -> None:
        super().__init__()
    ...
    def compute_loss(self: Any, targets: List[Dict[str, Tensor]], head_outputs: Dict[str, Tensor], anchors: List[Tensor], matched_idxs: List[Tensor]) -> Dict[str, Tensor]:
    ...
    def forward(self: Any, x: List[Tensor]) -> Dict[str, Tensor]:
...
class FCOSClassificationHead(nn.Module):
    ...
    def __init__(self: Any, in_channels: int, num_anchors: int, num_classes: int, num_convs: int=4, prior_probability: float=0.01, norm_layer: Optional[Callable[..., nn.Module]]=None) -> None:
        super().__init__()
    ...
    def forward(self: Any, x: List[Tensor]) -> Tensor:
        all_cls_logits = []
        for features in x:
...
class FCOSRegressionHead(nn.Module):
    ...
    def __init__(self: Any, in_channels: int, num_anchors: int, num_convs: int=4, norm_layer: Optional[Callable[..., nn.Module]]=None) -> object:
        super().__init__()
        if norm_layer is None:
            norm_layer = partial(nn.GroupNorm, 32)
        conv = []
    ...
    def forward(self: Any, x: List[Tensor]) -> Tuple[Tensor, Tensor]:
        all_bbox_regression = []
        all_bbox_ctrness = []
        for features in x:
...
class FCOS(nn.Module):
    ...
    def __init__(self: Any, backbone: nn.Module, num_classes: int, min_size: int=800, max_size: int=1333, image_mean: Optional[List[float]]=None, image_std: Optional[List[float]]=None, anchor_generator: Optional[AnchorGenerator]=None, head: Optional[nn.Module]=None, center_sampling_radius: float=1.5, score_thresh: float=0.2, nms_thresh: float=0.6, detections_per_img: int=100, topk_candidates: int=1000, **kwargs: Any) -> object:
        super().__init__()
    ...
    @torch.jit.unused
    def eager_outputs(self: Any, losses: Dict[str, Tensor], detections: List[Dict[str, Tensor]]) -> Tuple[Dict[str, Tensor], List[Dict[str, Tensor]]]:
        if self.training:
            return losses
        return detections

    def compute_loss(self: Any, targets: List[Dict[str, Tensor]], head_outputs: Dict[str, Tensor], anchors: List[Tensor], num_anchors_per_level: List[int]) -> Dict[str, Tensor]:
        matched_idxs = []
    ...
    def postprocess_detections(self: Any, head_outputs: Dict[str, List[Tensor]], anchors: List[List[Tensor]], image_shapes: List[Tuple[int, int]]) -> List[Dict[str, Tensor]]:
    ...
    def forward(self: Any, images: List[Tensor], targets: Optional[List[Dict[str, Tensor]]]=None) -> Tuple[Dict[str, Tensor], List[Dict[str, Tensor]]]:
        """
        Args:
        ...
        if torch.jit.is_scripting():
            if not self._has_warned:
                __jac_log_emit__(_gm_log_slot_0, ('FCOS always returns a (Losses, Detections) tuple in scripting',), {})
