...
from .utils import is_flash_attn_2_available, is_flash_attn_greater_or_equal, is_flash_attn_greater_or_equal_2_10, is_torch_npu_available, logging
...
def is_flash_attn_available() -> object:
...
def flash_attn_supports_top_left_mask() -> object:
...
def _upad_input(query_layer: torch.Tensor, key_layer: torch.Tensor, value_layer: torch.Tensor, attention_mask: torch.Tensor, query_length: int) -> object:
    """
...
def prepare_fa2_from_position_ids(query: Any, key: Any, value: Any, position_ids: Any) -> object:
    """
...
def fa_peft_integration_check(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, target_dtype: Optional[torch.dtype]=None) -> object:
    """
    ...
    if input_dtype == torch.float32:
        __jac_se_emit__(logger.warning_once, (f'The input hidden states seems to be silently casted in float32, this might be related to"\n            f" the fact you have upcasted embedding or layer norm layers in float32. We will cast back the input in"\n            f" {target_dtype}.',), {})
...
def _flash_attention_forward(query_states: torch.Tensor, key_states: torch.Tensor, value_states: torch.Tensor, attention_mask: Optional[torch.Tensor], query_length: int, is_causal: bool, dropout: float=0.0, position_ids: Optional[torch.Tensor]=None, softmax_scale: Optional[float]=None, sliding_window: Optional[int]=None, use_top_left_mask: bool=False, softcap: Optional[float]=None, deterministic: Optional[bool]=None, cu_seq_lens_q: Optional[torch.LongTensor]=None, cu_seq_lens_k: Optional[torch.LongTensor]=None, max_length_q: Optional[int]=None, max_length_k: Optional[int]=None, target_dtype: Optional[torch.dtype]=None, **kwargs: Any) -> object:
    ...
    if attention_mask is not None:
    ...
    elif position_ids is not None and (max_length_q is not None or (query_length != 1 and (not (torch.diff(position_ids, dim=-1) >= 0).all()))):
        ...
        attn_output = flash_attn_varlen_func(query_states, key_states, value_states, cu_seqlens_q=cu_seq_lens_q, cu_seqlens_k=cu_seq_lens_k, max_seqlen_q=max_length_q, max_seqlen_k=max_length_k, dropout_p=dropout, softmax_scale=softmax_scale, causal=causal, **flash_kwargs)
