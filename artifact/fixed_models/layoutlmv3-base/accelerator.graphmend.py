...
from jaclang.runtime.builtin import save
from typing import Callable
from jaclang.lib.jaclib import __jac_log_flush_hook__, __jac_se_emit__, __jac_se_flush__, __jac_se_region_open__, __jac_trap_guard__, destroy
import contextlib
import functools
import inspect
import json
...
if is_deepspeed_available():
    from .utils import DeepSpeedEngineWrapper, DeepSpeedOptimizerWrapper, DeepSpeedSchedulerWrapper, DummyOptim, DummyScheduler, map_pytorch_optim_to_deepspeed
    ...
    **Available attributes:**
    ...
    def __init__(self: Any, device_placement: bool=True, split_batches: bool=_split_batches, mixed_precision: PrecisionType | str | None=None, gradient_accumulation_steps: int=1, cpu: bool=False, dataloader_config: DataLoaderConfiguration | None=None, deepspeed_plugin: DeepSpeedPlugin | dict[str, DeepSpeedPlugin] | None=None, fsdp_plugin: FullyShardedDataParallelPlugin | None=None, torch_tp_plugin: TorchTensorParallelPlugin | None=None, megatron_lm_plugin: MegatronLMPlugin | None=None, rng_types: list[str | RNGType] | None=None, log_with: str | LoggerType | GeneralTracker | list[str | LoggerType | GeneralTracker] | None=None, project_dir: str | os.PathLike | None=None, project_config: ProjectConfiguration | None=None, gradient_accumulation_plugin: GradientAccumulationPlugin | None=None, step_scheduler_with_optimizer: bool=True, kwargs_handlers: list[KwargsHandler] | None=None, dynamo_backend: DynamoBackend | str | None=None, dynamo_plugin: TorchDynamoPlugin | None=None, deepspeed_plugins: DeepSpeedPlugin | dict[str, DeepSpeedPlugin] | None=None, parallelism_config: ParallelismConfig | None=None) -> object:
        ...
        if torch_tp_plugin is not None:
            warnings.warn('`TorchTensorParallelPlugin` is deprecated and will be removed in a future version of Accelerate. "\n                "Please use the `ParallelismConfig` with `tp_size` instead.', FutureWarning)
        ...
        if deepspeed_plugins is None:
            if AcceleratorState._shared_state != {} and AcceleratorState().distributed_type == DistributedType.DEEPSPEED:
                ...
                deepspeed_plugins = DeepSpeedPlugin() if os.environ.get('ACCELERATE_USE_DEEPSPEED', 'false').lower() == 'true' else None
        else:
            if AcceleratorState._shared_state != {} and AcceleratorState().distributed_type == DistributedType.DEEPSPEED and (AcceleratorState().deepspeed_plugins is not None):
                raise NotImplementedError('You cannot pass in a `deepspeed_plugin` when creating a second `Accelerator`. "\n                    "Please make sure the first `Accelerator` is initialized with all the plugins you want to use.')
        ...
        if os.environ.get('ACCELERATE_USE_FSDP', 'false').lower() == 'true' or isinstance(fsdp_plugin, FullyShardedDataParallelPlugin):
            if not is_torch_version('>=', FSDP_PYTORCH_VERSION):
                raise ValueError(f'FSDP requires PyTorch >= {FSDP_PYTORCH_VERSION}')
        if fsdp_plugin is None:
            fsdp_plugin = FullyShardedDataParallelPlugin() if os.environ.get('ACCELERATE_USE_FSDP', 'false').lower() == 'true' else None
        else:
        ...
        if megatron_lm_plugin is None:
            megatron_lm_plugin = MegatronLMPlugin() if os.environ.get('ACCELERATE_USE_MEGATRON_LM', 'false').lower() == 'true' else None
        else:
        ...
        self.state = AcceleratorState(mixed_precision=mixed_precision, cpu=cpu, dynamo_plugin=dynamo_plugin, deepspeed_plugin=deepspeed_plugins, fsdp_plugin=fsdp_plugin, megatron_lm_plugin=megatron_lm_plugin, parallelism_config=parallelism_config, _from_accelerator=True, **kwargs)
        ...
        if self.has_fp8_handler:
            if not self.fp8_enabled and self.distributed_type not in (DistributedType.FSDP, DistributedType.DEEPSPEED):
                raise ValueError("Passing in an FP8 configuration requires setting `mixed_precision='fp8'`.")
            self.delayed_fp8_autocast = self.fp8_backend == 'TE' and self.distributed_type in (DistributedType.MULTI_GPU, DistributedType.FSDP)
        if self.fp8_backend == FP8BackendType.AO and self.state.distributed_type == DistributedType.FSDP and self.state.fsdp_plugin.cpu_ram_efficient_loading:
            raise ValueError('torchao with FSDP2 and cpu_ram_efficient_loading is not supported, setting `cpu_ram_efficient_loading` to False will fix the issue and work as intended.')
        ...
        if self.state.mixed_precision == 'fp16' and self.device.type != 'cpu' and (self.distributed_type not in (DistributedType.DEEPSPEED, DistributedType.MEGATRON_LM)):
            ...
            if self.fp8_backend == FP8BackendType.MSAMP:
                if self.distributed_type == DistributedType.FSDP:
                    raise NotImplementedError('`accelerate` + `MS-AMP` + `FSDP` is not supported at this time. "\n                        "Please consider using deepspeed, which is supported.')
    ...
    @property
    def deepspeed_plugin(self: Any) -> object:
        """
    ...
    @property
    def use_distributed(self: Any) -> object:
        """
    ...
    @property
    def multi_device(self: Any) -> object:
        return self.use_distributed and self.distributed_type in (DistributedType.MULTI_GPU, DistributedType.MULTI_MLU, DistributedType.MULTI_SDAA, DistributedType.MULTI_MUSA, DistributedType.MULTI_NPU, DistributedType.MULTI_XPU, DistributedType.MULTI_HPU, DistributedType.MULTI_NEURON)

    @property
    def distributed_type(self: Any) -> object:
        return self.state.distributed_type

    @property
    def num_processes(self: Any) -> object:
        return self.state.num_processes

    @property
    def process_index(self: Any) -> object:
        return self.state.process_index

    @property
    def local_process_index(self: Any) -> object:
        return self.state.local_process_index

    @property
    def device(self: Any) -> object:
        return self.state.device

    @property
    def split_batches(self: Any) -> object:
        return self.dataloader_config.split_batches

    @property
    def dispatch_batches(self: Any) -> object:
        return self.dataloader_config.dispatch_batches

    @property
    def even_batches(self: Any) -> object:
        return self.dataloader_config.even_batches

    @even_batches.setter
    def even_batches(self: Any, value: bool) -> object:
        self.dataloader_config.even_batches = value

    @property
    def use_seedable_sampler(self: Any) -> object:
        return self.dataloader_config.use_seedable_sampler

    @property
    def non_blocking(self: Any) -> object:
        return self.dataloader_config.non_blocking

    @property
    def use_stateful_dataloader(self: Any) -> object:
        if hasattr(self.dataloader_config, 'use_stateful_dataloader'):
            return self.dataloader_config.use_stateful_dataloader
        return False

    @property
    def project_dir(self: Any) -> object:
        return self.project_configuration.project_dir

    @property
    def logging_dir(self: Any) -> object:
        return self.project_configuration.logging_dir

    @property
    def save_iteration(self: Any) -> object:
        return self.project_configuration.iteration

    @property
    def is_main_process(self: Any) -> object:
        """True for one process only."""
        return self.state.is_main_process

    @property
    def is_local_main_process(self: Any) -> object:
        """True for one process per server."""
        return self.state.is_local_main_process

    @property
    def is_last_process(self: Any) -> object:
        return self.process_index == self.num_processes - 1

    @property
    def mixed_precision(self: Any) -> object:
        return self.state.mixed_precision

    @property
    def is_fsdp2(self: Any) -> object:
        return self.state.is_fsdp2

    @property
    def is_composable_parallelism_enabled(self: Any) -> object:
        return self.is_fsdp2

    @property
    def parallelism_config(self: Any) -> Union[ParallelismConfig, None]:
        return self.state.parallelism_config

    @property
    def torch_device_mesh(self: Any) -> object:
        return self.state.device_mesh

    @property
    def should_save_model(self: Any) -> object:
        if (pc := self.parallelism_config) is None:
            return self.state.is_local_main_process
        _non_model_shard_dims = {pc.dp_replicate_enabled: 'dp_replicate', pc.cp_enabled: 'cp'}
        return True

    @property
    def tensor_parallel_rank(self: Any) -> int:
        """
    ...
    @property
    def pipeline_parallel_rank(self: Any) -> int:
        """
    ...
    @property
    def context_parallel_rank(self: Any) -> int:
        """
    ...
    @property
    def data_parallel_rank(self: Any) -> int:
        """
    ...
    @property
    def data_parallel_shard_rank(self: Any) -> int:
        """
    ...
    @contextmanager
    def split_between_processes(self: Any, inputs: list | tuple | dict | torch.Tensor, apply_padding: bool=False) -> object:
        """
    ...
    def on_main_process(self: Any, function: Callable[..., Any] | None=None) -> object:
        """
        ...
        def _inner(*args: Any, **kwargs: Any) -> object:
            return PartialState().on_main_process(function)(*args, **kwargs)
        return _inner

    def on_local_main_process(self: Any, function: Callable[..., Any] | None=None) -> object:
        """
    ...
    def on_last_process(self: Any, function: Callable[..., Any]) -> object:
        """
    ...
    def on_process(self: Any, function: Callable[..., Any] | None=None, process_index: int | None=None) -> object:
        """
    ...
    def on_local_process(self: Any, function: Callable[..., Any] | None=None, local_process_index: int | None=None) -> object:
        """
    ...
    @contextmanager
    def main_process_first(self: Any) -> object:
        """
    ...
    @contextmanager
    def local_main_process_first(self: Any) -> object:
        """
    ...
    @contextmanager
    def no_sync(self: Any, model: Any) -> object:
        """
    ...
    @staticmethod
    @contextmanager
    def trigger_sync_in_backward(model: Any) -> object:
    ...
    @property
    def sync_gradients(self: Any) -> object:
        return self.gradient_state.sync_gradients

    @sync_gradients.setter
    def sync_gradients(self: Any, sync_gradients: Any) -> object:
        self.gradient_state.sync_gradients = sync_gradients

    @property
    def gradient_accumulation_steps(self: Any) -> object:
        return self.gradient_state.num_steps

    @gradient_accumulation_steps.setter
    def gradient_accumulation_steps(self: Any, gradient_accumulation_steps: Any) -> object:
        self.gradient_state.plugin_kwargs.update({'num_steps': gradient_accumulation_steps})

    @contextmanager
    def accumulate(self: Any, *models: Any) -> object:
        """
    ...
    @contextmanager
    def join_uneven_inputs(self: Any, joinables: Any, even_batches: Any=None) -> object:
        """
        ...
        if self.multi_device:
            ...
            if even_batches is not None:
                ...
                if iterable_dl_seen:
                    warnings.warn('Overriding even_batches is only supported for map-style datasets, yet some dataloaders given were iterable')
            else:
    ...
    def print(self: Any, *args: Any, **kwargs: Any) -> object:
        """
    ...
    def _prepare_one(self: Any, obj: Any, first_pass: Any=False, device_placement: Any=None) -> object:
        if first_pass:
    ...
    def prepare(self: Any, *args: Any, device_placement: Any=None) -> object:
        """
        ...
        for obj in args:
            if isinstance(obj, torch.nn.Module) and self.verify_device_map(obj) and (self.distributed_type != DistributedType.NO) and (os.environ.get('ACCELERATE_BYPASS_DEVICE_MAP', 'false') != 'true'):
                raise ValueError('You can\'t train a model that has been loaded with `device_map=\'auto\'` in any distributed mode."\n                    " Please rerun your script specifying `--num_processes=1` or by launching with `python {{myscript.py}}`.')
        ...
        if self.is_fsdp2:
            ...
            if model_count < 1 and optimizer_count > 0 or (model_count > 0 and optimizer_count < 1):
                raise ValueError('When using FSDP2, a model and optimizer must be passed together to `Accelerator.prepare()`"\n                    " as the optimizer needs to have its parameters modified after the model is converted.')
            if model_count > 1:
    ...
    def _prepare_tp(self: Any, *args: Any) -> object:
        result = [self._prepare_one(obj, first_pass=True) if not isinstance(obj, torch.nn.Module) else obj for obj in args]
        ...
        for arg in args:
            ...
            if not any((isinstance(p, DTensor) for p in model.parameters())):
                logger.warning('The model parameters are not sharded by DTensor, we skip the TP preparation. If you are using "\n                    "a PreTrained model it is expected and this warning can be ignored.')
                return result
        ...
        def _get_tensor_address(p: Any) -> object:
    ...
    def _prepare_cp(self: Any, *args: Any) -> object:
    ...
    def _prepare_fsdp2(self: Any, *args: Any) -> object:
        result = [self._prepare_one(obj, first_pass=True) if not isinstance(obj, torch.nn.Module) else obj for obj in args]
        result = [self._prepare_one(obj) if not isinstance(obj, torch.nn.Module) else obj for obj in result]
        ...
        if self.state.dynamo_plugin.backend != DynamoBackend.NO and (not is_compiled_module(model)):
            if self.state.dynamo_plugin.use_regional_compilation:
                ...
                if hasattr(model, 'register_forward_pre_hook'):
                    model.register_forward_pre_hook(__jac_se_region_open__)
                    model.register_forward_hook(__jac_log_flush_hook__, always_call=True)
                model = __jac_trap_guard__(model)
            ...
            if isinstance(obj, torch.optim.Optimizer):
                for param_group in obj.param_groups:
                    for i, p in enumerate(param_group['params']):
                        ...
                        param_group['params'][i].data_ptr = p._local_tensor.data_ptr() if isinstance(p, DTensor) else p.data_ptr()
        self._models.append(model)
        model = fsdp2_prepare_model(self, model)
        if len(self._models) > 1 and self._models[-2] is self._models[-1]:
            destroy([self._models[-2]])
            del self._models[-2]
    ...
    def prepare_model(self: Any, model: torch.nn.Module, device_placement: bool | None=None, evaluation_mode: bool=False) -> object:
        """
            ...
            if len(model_devices) > 1 and self.distributed_type != DistributedType.NO:
                ...
                if self.device.type == 'cpu' and is_bitsandbytes_multi_backend_available():
                    ...
                    if self.device.index is not None or current_device_index != 0:
                        raise ValueError('You can\'t train a model that has been loaded in 8-bit or 4-bit precision on a different device than the one "\n                            "you\'re training on. Make sure you loaded the model on the correct device using for example `device_map={\'\':torch.cuda.current_device()}` or `device_map={\'\':torch.xpu.current_device()}`')
            if 'cpu' in model_devices and (not is_bitsandbytes_multi_backend_available()) or ('cpu' in model_devices and is_xpu_available()) or 'disk' in model_devices:
                raise ValueError('You can\'t train a model that has been loaded in 8-bit or 4-bit precision with CPU or disk offload. "\n                    "If you want train the 8-bit or 4-bit model in CPU, please install bitsandbytes with multi-backend, see https://huggingface.co/docs/bitsandbytes/main/en/installation#multi-backend')
        ...
        if not evaluation_mode:
            if self.multi_device and (not (self.parallelism_config and self.parallelism_config.tp_enabled)):
                if model_has_dtensor(model):
                    raise ValueError("Your model contains `DTensor` parameters, which is incompatible with DDP. Maybe you loaded your model with `device_map='auto'`? Specify `device_map='cuda'` or 'xpu' or 'cpu' instead.")
                ...
                if not hasattr(model, 'tp_size'):
                    raise NotImplementedError('Model should undergo tensor parallel before passing it to accelerate."\n                        "You can use .from_pretrained(..., tp_plan=\'auto\') if the model supports')
                if model.tp_size != self.parallelism_config.tp_size:
                    raise ValueError(f"tp_size in the plugin {self.parallelism_config.tp_size} should be same as model's tp size {model.tp_size}")
            elif self.is_fsdp2:
                raise ValueError('FSDP2 preparation should be done via `accelerate.prepare()`, as it requires a model and an optimizer.')
                ...
                if not is_type_fsdp:
                    ...
                    fsdp_plugin.param_init_fn = ensure_weights_retied(fsdp_plugin.param_init_fn, model, self.device)
                ...
                if self.mixed_precision != 'no':
                    ...
                    if self.is_main_process:
                        for name_log, param_log in upcasted_log:
                            __jac_se_emit__(warnings.warn, (f'Upcasted low precision parameters in {name_log} because mixed precision turned on in FSDP. "\n                                f"Affects: {param_log}.',), {})
                        if len(upcasted_log) > 0:
                            __jac_se_emit__(warnings.warn, ('FSDP upcast of low precision parameters may affect the precision of model checkpoints.',), {})
                if len(self._models) > 1 and self._models[-2] is self._models[-1]:
                    destroy([self._models[-2]])
                    del self._models[-2]
    ...
    def _prepare_ao(self: Any, *args: Any) -> object:
        if not is_torchao_available():
            raise ImportError('`torchao` was not found on your system or is too old of a version. Please ensure that `torchao >= 0.6.1` is installed')
        if self.is_fsdp2:
            models = [x for x in args if isinstance(x, torch.nn.Module)]
            optimizers = [x for x in args if isinstance(x, torch.optim.Optimizer)]
        for arg in args:
            if isinstance(arg, torch.nn.Module):
                convert_model_to_fp8_ao(arg, config=self.ao_recipe_handler.config, module_filter_func=self.ao_recipe_handler.module_filter_func)
        if self.is_fsdp2 and len(optimizers) > 0 and self.ao_recipe_handler.config.enable_fsdp_float8_all_gather:
            from torchao.float8 import precompute_float8_dynamic_scale_for_fsdp
            optimizers[0].register_step_post_hook(lambda *args: Any, **kwargs: Any: precompute_float8_dynamic_scale_for_fsdp(models[0]))
        return args

    def _prepare_te(self: Any, *args: Any) -> object:
    ...
    def _prepare_deepspeed(self: Any, *args: Any) -> object:
        import deepspeed
        ...
        if deepspeed_plugin.is_auto('train_micro_batch_size_per_gpu'):
            if is_dataloader_present:
                ...
                if any((bs is None for bs in batch_sizes)):
                    raise ValueError('At least one of the dataloaders passed to `accelerate.prepare()` has `None` as batch size. "\n                        "Please set an integer value in `train_micro_batch_size_per_gpu` in the deepspeed config file "\n                        "or assign integer value to `AcceleratorState().deepspeed_plugin.deepspeed_config[\'train_micro_batch_size_per_gpu\']`.')
                ...
                raise ValueError('When using DeepSpeed, `accelerate.prepare()` requires you to pass at least one of training or evaluation dataloaders "\n                    "with `batch_size` attribute returning an integer value "\n                    "or alternatively set an integer value in `train_micro_batch_size_per_gpu` in the deepspeed config file "\n                    "or assign integer value to `AcceleratorState().deepspeed_plugin.deepspeed_config[\'train_micro_batch_size_per_gpu\']`.')
        else:
        ...
        if batch_size_per_device is not None:
            ...
            config_kwargs['train_batch_size'] = batch_size_per_device * deepspeed_plugin.get_value('gradient_accumulation_steps') * self.num_processes // sp_size
        model = None
        optimizer = None
        scheduler = None
        for obj in result:
        ...
        if optimizer is not None:
            if 'optimizer' in deepspeed_plugin.deepspeed_config and (not isinstance(optimizer, DummyOptim)):
                raise ValueError('You cannot specify an optimizer in the config file and in the code at the same time. "\n                    "Please remove the optimizer from the config file or "\n                    "create `accelerate.utils.DummyOptim` in the code.')
        ...
        if scheduler is not None:
            if 'scheduler' in deepspeed_plugin.deepspeed_config and (not isinstance(scheduler, DummyScheduler)):
                raise ValueError('You cannot specify a scheduler in the config file and in the code at the same time. "\n                    "Please remove the scheduler from the config file or "\n                    "create `accelerate.utils.DummyScheduler` in the code.')
                ...
                raise ValueError('Either specify a scheduler in the config file or "\n                    "pass in the `lr_scheduler_callable` parameter when using `accelerate.utils.DummyScheduler`.')
        if optimizer is not None and scheduler is not None:
            if isinstance(optimizer, DummyOptim) and (not isinstance(scheduler, DummyScheduler)):
                raise ValueError('You can only specify `accelerate.utils.DummyScheduler` in the code when using "\n                    "`accelerate.utils.DummyOptim`.')
        if model is not None:
                ...
                if isinstance(optimizer, DummyOptim):
                    ...
                    if self.deepspeed_config['zero_optimization'].get('offload_optimizer', {}).get('device', 'none') != 'none' and self.deepspeed_config.get('zero_force_ds_cpu_optimizer', True):
                        if self.device.type == 'hpu' and os.environ.get('PT_HPU_LAZY_MODE', '1') == '1':
                            raise ValueError('You can\'t use an Offload Optimizer with HPU in Lazy Mode. "\n                                "Please set the environment variable `PT_HPU_LAZY_MODE` to `0`.')
            ...
            if sp_size > 1:
                if sp_backend != 'deepspeed':
                    raise ValueError(f"In order to use the configured {{sp_size={sp_size!r} with DeepSpeed, you need to configure sp_backend='deepspeed', yet you configured it to be {{sp_backend={sp_backend!r}.")
                ver_min_required = '0.18.2'
                if not compare_versions('deepspeed', '>=', ver_min_required):
                    raise ImportError(f'Deepspeed ALST/Ulysses requires deepspeed>={ver_min_required}. Please update DeepSpeed via `pip install deepspeed -U`.')
                from deepspeed.runtime.sequence_parallel.ulysses_sp import UlyssesSPAttentionHF, UlyssesSPDataLoaderAdapter
                if not hasattr(model, 'config'):
                    raise ValueError("UlyssesSPAttentionHF currently works with HF Transformers and expects the model object to have a config attribute but this model doesn't have one.")
                kwagrs = {}
                ...
                mpu = UlyssesSPAttentionHF.register_with_transformers(model_name_or_path=model, sequence_parallel_size=sp_size, seq_length=sp_handler.sp_seq_length, seq_length_is_variable=sp_handler.sp_seq_length_is_variable, core_attn_implementation=sp_handler.sp_attn_implementation, micro_batch_size=batch_size_per_device, **kwagrs)
                kwargs['mpu'] = mpu
                for i in range(len(result)):
                    if isinstance(result[i], torch.utils.data.DataLoader):
                        if sp_size > 1:
                            ...
                            result[i] = UlyssesSPDataLoaderAdapter(result[i], sp_rank=sp_rank, sp_group=sp_group, sp_world_size=sp_world_size, device=self.device)
    ...
    def deepspeed_ulysses_dl_adapter(self: Any, dl: Any, model: Any) -> object:
        ...
        dl = UlyssesSPDataLoaderAdapter(dl, sp_rank=sp_rank, sp_group=sp_group, sp_world_size=sp_world_size, device=model.device)
        return dl

    def _prepare_megatron_lm(self: Any, *args: Any) -> object:
        ...
        if micro_batch_size is not None:
            ...
            raise ValueError('When you do not pass the dataloader parameter, the `data_parallel_size`, "\n                "`micro_batch_size`, and `global_batch_size` megatron parameters will not be updated.')
        model = None
        optimizer = None
        scheduler = None
        batch_data = None
    ...
    def _prepare_device_mesh(self: Any) -> object:
        """
    ...
    def _prepare_msamp(self: Any, *args: Any, device_placement: Any) -> object:
        warnings.warn('MS-AMP is deprecated and will be removed in a future version of Accelerate. "\n            "Please use `\'te\'` (Transformer Engine) or `\'torchao\'` as the backend for FP8 "\n            "mixed precision training instead.', FutureWarning)
        if not is_msamp_available():
            raise ImportError('MS-AMP was not found on your system. Please ensure that MS-AMP is available "\n                " or choose `\'te\'` as the backend for FP8 mixed precision training.')
        import msamp
        model, optimizer = (None, None)
    ...
    def prepare_data_loader(self: Any, data_loader: torch.utils.data.DataLoader, device_placement: Any=None, slice_fn_for_dispatch: Any=None) -> object:
        """
    ...
    def prepare_scheduler(self: Any, scheduler: LRScheduler) -> object:
        """
    ...
    def backward(self: Any, loss: Any, **kwargs: Any) -> object:
        """
    ...
    def set_trigger(self: Any) -> object:
        """
    ...
    def check_trigger(self: Any) -> object:
        """
    ...
    def unscale_gradients(self: Any, optimizer: Any=None) -> object:
        """
    ...
    def clip_grad_norm_(self: Any, parameters: Any, max_norm: Any, norm_type: Any=2) -> object:
        """
    ...
    def clip_grad_value_(self: Any, parameters: Any, clip_value: Any) -> object:
        """
    ...
    def gather(self: Any, tensor: Any) -> object:
        """
    ...
    def gather_for_metrics(self: Any, input_data: Any, use_gather_object: Any=False) -> object:
        """
        ...
        try:
            if self.gradient_state.end_of_dataloader:
                if self.gradient_state.remainder == -1:
                    ...
                    def _adjust_samples(tensor: Any) -> object:
    ...
    def reduce(self: Any, tensor: Any, reduction: Any='sum', scale: Any=1.0) -> object:
        """
    ...
    def pad_across_processes(self: Any, tensor: Any, dim: Any=0, pad_index: Any=0, pad_first: Any=False) -> object:
        """
    ...
    def unwrap_model(self: Any, model: Any, keep_fp32_wrapper: bool=True, keep_torch_compile: bool=True) -> object:
        """
    ...
    def wait_for_everyone(self: Any) -> object:
        """
    ...
    @on_main_process
    def init_trackers(self: Any, project_name: str, config: dict | None=None, init_kwargs: dict | None=None) -> object:
        """
    ...
    def get_tracker(self: Any, name: str, unwrap: bool=False) -> object:
        """
    ...
    @on_main_process
    def log(self: Any, values: dict, step: int | None=None, log_kwargs: dict | None=None) -> object:
        """
    ...
    def end_training(self: Any) -> object:
        """
    ...
    def save(self: Any, obj: Any, f: Any, safe_serialization: Any=False) -> object:
        """
    ...
    def save_model(self: Any, model: torch.nn.Module, save_directory: Union[str, os.PathLike], max_shard_size: Union[int, str]='10GB', safe_serialization: bool=True) -> object:
        """
    ...
    def register_save_state_pre_hook(self: Any, hook: Callable[..., None]) -> hooks.RemovableHandle:
        """
    ...
    def save_state(self: Any, output_dir: str | None=None, safe_serialization: bool=True, **save_model_func_kwargs: Any) -> object:
        """
    ...
    def register_load_state_pre_hook(self: Any, hook: Callable[..., None]) -> hooks.RemovableHandle:
        """
    ...
    def load_state(self: Any, input_dir: str | None=None, load_kwargs: dict | None=None, **load_model_func_kwargs: Any) -> object:
        """
        ...
        if map_location is None:
            if self.num_processes > 1 and self.multi_device and (self.distributed_type != DistributedType.MULTI_XPU):
                ...
                map_location = 'cpu'
        override_attributes = load_accelerator_state(input_dir, models, optimizers, schedulers, dataloaders, self.state.process_index, scaler, map_location, load_kwargs, **load_model_func_kwargs)
    ...
    def free_memory(self: Any, *objects: Any) -> object:
        """
    ...
    def clear(self: Any, *objects: Any) -> object:
        """
    ...
    def _get_named_parameters(self: Any, *args: Any, drop_refs: Any=False) -> object:
        named_parameters = {}
        accessor_mapping = {}
        for obj in args:
            if isinstance(obj, torch.nn.Module):
                ...
                _is_dtensor_available = _torch_distributed_available and is_torch_version('>=', DTENSOR_PYTORCH_VERSION)
                if _is_dtensor_available:
                    from torch.distributed.tensor import DTensor
                    accessor_mapping[DTensor] = '_local_tensor'
                named_parameters.update({n: getattr(p, accessor_mapping[type(p)]).data_ptr() if type(p) in accessor_mapping else p.data_ptr() for n, p in obj.named_parameters()})
        return named_parameters

    def _get_devices(self: Any, *args: Any) -> object:
        model_device = None
    ...
    def get_state_dict(self: Any, model: Any, unwrap: Any=True) -> object:
        """
            ...
            if zero3_sharding or tp_sharding:
                if model.zero_gather_16bit_weights_on_model_save():
                    ...
                    if tp_sharding and (not compare_versions('deepspeed', '>=', ver_min_required)):
                        raise ImportError(f'Deepspeed TP requires deepspeed>={ver_min_required}. Please update DeepSpeed via `pip install deepspeed -U`.')
                    state_dict = model._consolidated_16bit_state_dict() if tp_sharding else model._zero3_consolidated_16bit_state_dict()
                else:
                    raise ValueError('Cannot get 16bit model weights because `stage3_gather_16bit_weights_on_model_save` in DeepSpeed config is False. "\n                        "To save the model weights in 16bit, set `stage3_gather_16bit_weights_on_model_save` to True in DeepSpeed config file or "\n                        "set `zero3_save_16bit_model` to True when using `accelerate config`. "\n                        "To save the full checkpoint, run `model.save_checkpoint(save_dir)` and use `zero_to_fp32.py` to recover weights.')
            else:
    ...
    def register_for_checkpointing(self: Any, *objects: Any) -> object:
        """
    ...
    @contextmanager
    def maybe_context_parallel(self: Any, buffers: list[torch.Tensor] | None=None, buffer_seq_dims: list[int] | None=None, no_restore_buffers: set[torch.Tensor] | None=None) -> object:
        """
        ...
        if self.parallelism_config and self.parallelism_config.cp_backend == 'torch' and self.parallelism_config.cp_enabled:
            with self._cp_context(buffers=buffers, buffer_seq_dims=buffer_seq_dims, no_restore_buffers=no_restore_buffers):
                yield
        else:
            logger.warning_once('Context parallel training is not enabled. This context manager will have no effect. "\n                "To enable it, set `parallelism_config.cp_size` > 1 in the `Accelerator` constructor.')
            yield

    @contextmanager
    def autocast(self: Any, autocast_handler: AutocastKwargs=None) -> object:
        """
    ...
    @contextmanager
    def profile(self: Any, profile_handler: ProfileKwargs | None=None) -> object:
        """
    ...
    @property
    def optimizer_step_was_skipped(self: Any) -> object:
        """
    ...
    def skip_first_batches(self: Any, dataloader: Any, num_batches: int=0) -> object:
        """
    ...
    def lomo_backward(self: Any, loss: torch.Tensor, learning_rate: float) -> None:
        """
    ...
    @property
    def fp8_backend(self: Any) -> FP8BackendType:
