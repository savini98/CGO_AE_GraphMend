...
from jaclang.lib.jaclib import __jac_se_emit__, destroy
import collections
import copy
import functools
import gc
...
from .integrations.tensor_parallel import ALL_PARALLEL_STYLES, _get_parameter_tp_plan, initialize_tensor_parallelism, repack_weights, replace_state_dict_local_with_dtensor, shard_and_distribute_module, verify_tp_plan
...
from .utils import ADAPTER_SAFE_WEIGHTS_NAME, ADAPTER_WEIGHTS_NAME, CONFIG_NAME, DUMMY_INPUTS, FLAX_WEIGHTS_NAME, SAFE_WEIGHTS_INDEX_NAME, SAFE_WEIGHTS_NAME, TF2_WEIGHTS_NAME, TF_WEIGHTS_NAME, WEIGHTS_INDEX_NAME, WEIGHTS_NAME, ContextManagers, ModelOutput, PushToHubMixin, cached_file, check_torch_load_is_safe, copy_func, download_url, extract_commit_hash, has_file, is_accelerate_available, is_bitsandbytes_available, is_flash_attn_2_available, is_kernels_available, is_offline_mode, is_optimum_available, is_peft_available, is_remote_url, is_safetensors_available, is_torch_flex_attn_available, is_torch_greater_or_equal, is_torch_mlu_available, is_torch_npu_available, is_torch_sdpa_available, is_torch_xla_available, is_torch_xpu_available, logging, replace_return_docstrings, strtobool
...
if is_accelerate_available():
    ...
    from accelerate.utils import check_tied_parameters_on_same_device, extract_model_from_parallel, get_balanced_memory, get_max_memory, load_offloaded_weights, offload_weight, save_offload_index
...
def is_local_dist_rank_0() -> object:
...
VLMS = ['aria', 'ayavision', 'emu3', 'fuyu', 'gotocr2', 'gemma3', 'internvl', 'llava', 'mistral3', 'mllama', 'paligemma', 'qwen2vl', 'qwen2_5_vl', 'videollava', 'vipllava']

@contextmanager
def no_init_weights() -> object:
    """
    ...
    def _skip_init(*args: Any, **kwargs: Any) -> object:
        pass
...
@contextmanager
def set_quantized_state() -> object:
...
def restore_default_torch_dtype(func: Any) -> object:
    """
...
def get_torch_context_manager_or_global_device() -> object:
    """
...
def get_parameter_device(parameter: Union[nn.Module, 'ModuleUtilsMixin']) -> object:
    try:
...
def get_parameter_dtype(parameter: Union[nn.Module, 'ModuleUtilsMixin']) -> object:
    """
...
def get_state_dict_dtype(state_dict: Any) -> object:
    """
...
def load_sharded_checkpoint(model: Any, folder: Any, strict: Any=True, prefer_safe: Any=True) -> object:
    """
    This is the same as
...
def load_state_dict(checkpoint_file: Union[str, os.PathLike], is_quantized: bool=False, map_location: Optional[Union[str, torch.device]]='cpu', weights_only: bool=True) -> object:
    """
    ...
    if checkpoint_file.endswith('.safetensors') and is_safetensors_available():
        with safe_open(checkpoint_file, framework='pt') as f:
            ...
            if metadata is not None and metadata.get('format') not in ['pt', 'tf', 'flax', 'mlx']:
                raise OSError(f'The safetensors archive passed at {checkpoint_file} does not contain the valid metadata. Make sure "\n                    "you save your model with the `save_pretrained` method."')
            state_dict = {}
            for k in f.keys():
    ...
    try:
        ...
        try:
            with open(checkpoint_file) as f:
                if f.read(7) == 'version':
                    raise OSError('You seem to have cloned a repository without having git-lfs installed. Please install "\n                        "git-lfs and run `git lfs install` followed by `git lfs pull` in the folder "\n                        "you cloned.')
                else:
                    raise e from ValueError(f'Unable to locate the file {checkpoint_file} which is necessary to load this pretrained "\n                        "model. Make sure you have saved the model properly."')
        except (UnicodeDecodeError, ValueError):
            raise OSError(f'''Unable to load weights from pytorch checkpoint file for '{checkpoint_file}' "\n                f"at '{checkpoint_file}'. "\n                "If you tried to load a PyTorch model from a TF 2.0 checkpoint, please set from_tf=True."''')

def set_initialized_submodules(model: Any, state_dict_keys: Any) -> object:
    """
...
def _get_tied_weight_keys(module: nn.Module, prefix: Any='') -> object:
    tied_weight_keys = []
...
def _load_parameter_into_model(model: 'PreTrainedModel', param_name: str, tensor: torch.Tensor) -> object:
    """Cast a single parameter `param_name` into the `model`, with value `tensor`."""
    module, param_type = get_module_from_name(model, param_name)
    module.load_state_dict({param_type: tensor}, strict=False, assign=True)

@torch.no_grad()
def _load_state_dict_into_meta_model(model: 'PreTrainedModel', state_dict: Dict, shard_file: str, expected_keys: List[str], reverse_renaming_mapping: Dict[str, str], device_map: Optional[Dict]=None, disk_offload_folder: Optional[str]=None, disk_offload_index: Optional[Dict]=None, cpu_offload_folder: Optional[str]=None, cpu_offload_index: Optional[Dict]=None, hf_quantizer: Optional[HfQuantizer]=None, is_safetensors: bool=False, keep_in_fp32_regex: Optional[re.Pattern]=None, unexpected_keys: Optional[List[str]]=None, device_mesh: Optional['torch.distributed.device_mesh.DeviceMesh']=None) -> Tuple[Optional[Dict], Optional[Dict]]:
    ...
    is_hqq_or_bnb = is_quantized and hf_quantizer.quantization_config.quant_method in {QuantizationMethod.HQQ, QuantizationMethod.BITS_AND_BYTES}
    ...
    for param_name, empty_param in state_dict.items():
        ...
        if device_mesh is not None:
            shard_and_distribute_module(model, param, empty_param, param_name, casting_dtype, to_contiguous, device_mesh.get_local_rank(), device_mesh)
        else:
            param = param[...]
...
def _get_resolved_checkpoint_files(pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], subfolder: str, variant: Optional[str], gguf_file: Optional[str], from_tf: bool, from_flax: bool, use_safetensors: bool, cache_dir: str, force_download: bool, proxies: Optional[Dict[str, str]], local_files_only: bool, token: Optional[Union[str, bool]], user_agent: dict, revision: str, commit_hash: Optional[str], transformers_explicit_filename: Optional[str]=None) -> Tuple[Optional[List[str]], Optional[Dict]]:
    ...
    if pretrained_model_name_or_path is not None and gguf_file is None:
        ...
        if is_local:
            if transformers_explicit_filename is not None:
            ...
            elif from_tf and os.path.isfile(os.path.join(pretrained_model_name_or_path, subfolder, TF_WEIGHTS_NAME + '.index')):
                ...
                raise EnvironmentError(f'Error no file named {_add_variant(WEIGHTS_NAME, variant)} found in directory"\n                    f" {pretrained_model_name_or_path} but there is a file for TensorFlow weights. Use"\n                    " `from_tf=True` to load this model from those weights."')
                ...
                raise EnvironmentError(f'Error no file named {_add_variant(SAFE_WEIGHTS_NAME, variant)} found in directory"\n                    f" {pretrained_model_name_or_path}.')
            else:
                raise EnvironmentError(f"""Error no file named {_add_variant(WEIGHTS_NAME, variant)}, {_add_variant(SAFE_WEIGHTS_NAME, variant)},"\n                    f" {TF2_WEIGHTS_NAME}, {TF_WEIGHTS_NAME + '.index'} or {FLAX_WEIGHTS_NAME} found in directory"\n                    f" {pretrained_model_name_or_path}.""")
        elif os.path.isfile(os.path.join(subfolder, pretrained_model_name_or_path)):
            archive_file = pretrained_model_name_or_path
            is_local = True
        elif os.path.isfile(os.path.join(subfolder, pretrained_model_name_or_path + '.index')):
            if not from_tf:
                raise ValueError(f'''We found a TensorFlow checkpoint at {pretrained_model_name_or_path + '.index'}, please set "\n                    "from_tf to True to load from this checkpoint."''')
            ...
            try:
                cached_file_kwargs = {'cache_dir': cache_dir, 'force_download': force_download, 'proxies': proxies, 'local_files_only': local_files_only, 'token': token, 'user_agent': user_agent, 'revision': revision, 'subfolder': subfolder, '_raise_exceptions_for_gated_repo': False, '_raise_exceptions_for_missing_entries': False, '_commit_hash': commit_hash}
                ...
                if resolved_archive_file is None and filename == _add_variant(SAFE_WEIGHTS_NAME, variant):
                    ...
                    if resolved_archive_file is not None:
                        ...
                        if resolved_archive_file is None:
                            raise EnvironmentError(f'{pretrained_model_name_or_path} does not appear to have a file named"\n                                f" {_add_variant(SAFE_WEIGHTS_NAME, variant)} or {_add_variant(SAFE_WEIGHTS_INDEX_NAME, variant)} "\n                                "and thus cannot be loaded with `safetensors`. Please make sure that the model has "\n                                "been saved with `safe_serialization=True` or do not set `use_safetensors=True`."')
                    else:
                        filename = _add_variant(WEIGHTS_NAME, variant)
                        resolved_archive_file = cached_file(pretrained_model_name_or_path, filename, **cached_file_kwargs)
                ...
                if not local_files_only and (not is_offline_mode()):
                    if resolved_archive_file is not None:
                        if filename in [WEIGHTS_NAME, WEIGHTS_INDEX_NAME]:
                            ...
                            cached_file_kwargs = {'cache_dir': cache_dir, 'force_download': force_download, 'local_files_only': local_files_only, 'user_agent': user_agent, 'subfolder': subfolder, '_raise_exceptions_for_gated_repo': False, '_raise_exceptions_for_missing_entries': False, '_commit_hash': commit_hash, **has_file_kwargs}
                            if not has_file(pretrained_model_name_or_path, safe_weights_name, **has_file_kwargs):
                                Thread(target=auto_conversion, args=(pretrained_model_name_or_path,), kwargs={'ignore_errors_during_conversion': True, **cached_file_kwargs}, name='Thread-auto_conversion').start()
                    else:
                        has_file_kwargs = {'revision': revision, 'proxies': proxies, 'token': token, 'cache_dir': cache_dir, 'local_files_only': local_files_only}
                ...
                raise e from EnvironmentError(f"""Can't load the model for '{pretrained_model_name_or_path}'. If you were trying to load it"\n                    " from 'https://huggingface.co/models', make sure you don't have a local directory with the"\n                    f" same name. Otherwise, make sure '{pretrained_model_name_or_path}' is the correct path to a"\n                    f" directory containing a file named {_add_variant(WEIGHTS_NAME, variant)},"\n                    f" {TF2_WEIGHTS_NAME}, {TF_WEIGHTS_NAME} or {FLAX_WEIGHTS_NAME}.""")
        if is_local:
...
def _get_torch_dtype(cls: Any, torch_dtype: Optional[Union[str, torch.dtype, Dict]], checkpoint_files: Optional[List[str]], config: PretrainedConfig, sharded_metadata: Optional[Dict], state_dict: Optional[Dict], weights_only: bool) -> Tuple[PretrainedConfig, Optional[torch.dtype], Optional[torch.dtype]]:
    ...
    if torch_dtype is not None:
        if isinstance(torch_dtype, str):
            if torch_dtype == 'auto':
                if hasattr(config, 'torch_dtype') and config.torch_dtype is not None:
                    ...
                    if is_sharded and 'dtype' in sharded_metadata:
                        ...
                        torch_dtype = get_state_dict_dtype(state_dict)
                    logger.info('Since the `torch_dtype` attribute can\'t be found in model\'s config object, "\n                        "will use torch_dtype={torch_dtype} as derived from model\'s weights')
            ...
            raise ValueError(f"""`torch_dtype` can be one of: `torch.dtype`, `'auto'`, a string of a valid `torch.dtype` or a `dict` with valid `torch_dtype` "\n                f"for each sub-config in composite configs, but received {torch_dtype}""")
...
def _get_device_map(model: 'PreTrainedModel', device_map: Optional[Union[str, Dict]], max_memory: Optional[Dict], hf_quantizer: Optional[HfQuantizer], torch_dtype: Optional[torch.dtype], keep_in_fp32_regex: Optional[re.Pattern]) -> Dict:
    ...
    if isinstance(device_map, str):
        ...
        if keep_in_fp32_regex is not None:
            special_dtypes.update({name: torch.float32 for name, _ in model.named_parameters() if keep_in_fp32_regex.search(name)})
        ...
        if 'special_dtypes' in inspect.signature(infer_auto_device_map).parameters:
            ...
            logger.warning('This model has some weights that should be kept in higher precision, you need to upgrade "\n                "`accelerate` to properly deal with them (`pip install --upgrade accelerate`).')
        if device_map != 'sequential':
            inferred_max_memory = get_balanced_memory(model, dtype=target_dtype, low_zero=device_map == 'balanced_low_0', max_memory=max_memory, **device_map_kwargs)
        else:
...
def _find_missing_and_unexpected_keys(cls: Any, model: 'PreTrainedModel', original_checkpoint_keys: List[str], checkpoint_keys: List[str], loading_base_model_from_task_state_dict: bool, hf_quantizer: Optional[HfQuantizer], device_map: Dict) -> Tuple[List[str], List[str]]:
...
def _find_mismatched_keys(model: 'PreTrainedModel', state_dict: Optional[Dict], checkpoint_files: Optional[List[str]], ignore_mismatched_sizes: bool, keys_to_rename_mapping: Dict[str, str], is_quantized: bool, weights_only: bool) -> Tuple[List[str], List[Tuple[int, int]]]:
    ...
    for shard_file in checkpoint_files:
        if shard_file != '':
            state_dict = load_state_dict(shard_file, is_quantized=is_quantized, map_location='meta', weights_only=weights_only)
        ...
        for key in new_state_dict.keys():
            if key in model_state_dict and new_state_dict[key].shape != model_state_dict[key].shape:
                if not (new_state_dict[key].shape[-1] == 1 and new_state_dict[key].numel() * 2 == model_state_dict[key].numel()):
...
class ModuleUtilsMixin:
    ...
    @staticmethod
    def _hook_rss_memory_pre_forward(module: Any, *args: Any, **kwargs: Any) -> object:
        try:
            import psutil
        except ImportError:
    ...
    @staticmethod
    def _hook_rss_memory_post_forward(module: Any, *args: Any, **kwargs: Any) -> object:
        try:
            import psutil
        except ImportError:
            raise ImportError('You need to install psutil (pip install psutil) to use memory tracing.')
    ...
    def add_memory_hooks(self: Any) -> object:
        """
    ...
    def reset_memory_hooks_state(self: Any) -> object:
        """
    ...
    @property
    def dtype(self: Any) -> torch.dtype:
        """
        `torch.dtype`: The dtype of the module (assuming that all the module parameters have the same dtype).
        """
        return get_parameter_dtype(self)

    def invert_attention_mask(self: Any, encoder_attention_mask: Tensor) -> Tensor:
        """
    ...
    @staticmethod
    def create_extended_attention_mask_for_decoder(input_shape: Any, attention_mask: Any, device: Any=None) -> object:
    ...
    def get_extended_attention_mask(self: Any, attention_mask: Tensor, input_shape: Tuple[int], device: torch.device=None, dtype: torch.float=None) -> Tensor:
        """
        ...
        if not (attention_mask.dim() == 2 and self.config.is_decoder):
            if device is not None:
                __gm_se_arg_0 = FutureWarning
                __jac_se_emit__(warnings.warn, ('The `device` argument is deprecated and will be removed in v5 of Transformers.', __gm_se_arg_0.clone() if hasattr(__gm_se_arg_0, 'clone') else __gm_se_arg_0), {})
    ...
    def get_head_mask(self: Any, head_mask: Optional[Tensor], num_hidden_layers: int, is_attention_chunked: bool=False) -> Tensor:
        """
    ...
    def _convert_head_mask_to_5d(self: Any, head_mask: Any, num_hidden_layers: Any) -> object:
    ...
    def num_parameters(self: Any, only_trainable: bool=False, exclude_embeddings: bool=False) -> int:
        """
        ...
        if is_loaded_in_4bit:
            if is_bitsandbytes_available():
                ...
                raise ValueError('bitsandbytes is not installed but it seems that the model has been loaded in 4bit precision, something went wrong"\n                    " make sure to install bitsandbytes with `pip install bitsandbytes`. You also need a GPU. ')
    ...
    def estimate_tokens(self: Any, input_dict: Dict[str, Union[torch.Tensor, Any]]) -> int:
        """
    ...
    def floating_point_ops(self: Any, input_dict: Dict[str, Union[torch.Tensor, Any]], exclude_embeddings: bool=True) -> int:
        """
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, PushToHubMixin, PeftAdapterMixin):
    ...
    @property
    def dummy_inputs(self: Any) -> Dict[str, torch.Tensor]:
        """
    ...
    @property
    def framework(self: Any) -> str:
        """
        :str: Identifies that this is a PyTorch model.
        """
        return 'pt'

    def __init__(self: Any, config: PretrainedConfig, *inputs: Any, **kwargs: Any) -> object:
        super().__init__()
        ...
        if self._keep_in_fp32_modules is not None:
            ...
            for module in self._keep_in_fp32_modules:
                if module not in unique_module_names:
                    raise ValueError(f'{module} was specified in the `_keep_in_fp32_modules` list, but is not part of the modules in"\n                        f" {self.__class__.__name__}')
    ...
    def dequantize(self: Any) -> object:
        """
    ...
    def _backward_compatibility_gradient_checkpointing(self: Any) -> object:
        if self.supports_gradient_checkpointing and getattr(self.config, 'gradient_checkpointing', False):
            self.gradient_checkpointing_enable()
            delattr(self.config, 'gradient_checkpointing')

    def add_model_tags(self: Any, tags: Union[List[str], str]) -> None:
        '''r"""
    ...
    def _from_config(cls: Any, config: Any, **kwargs: Any) -> object:
        """
    ...
    @classmethod
    def _autoset_attn_implementation(cls: Any, config: Any, use_flash_attention_2: bool=False, torch_dtype: Optional[torch.dtype]=None, device_map: Optional[Union[str, Dict[str, int]]]=None, check_device_map: bool=True) -> object:
        """
        ...
        if hasattr(config, '_attn_implementation_internal') and config._attn_implementation_internal is not None:
            if config._attn_implementation != 'flash_attention_2' and use_flash_attention_2:
                raise ValueError(f"""Both attn_implementation="{config._attn_implementation}" and `use_flash_attention_2=True` were used when loading the model, which are not compatible.'\n                    ' We recommend to just use `attn_implementation="flash_attention_2"` when loading the model.'""")
            if isinstance(config._attn_implementation, str) and re.match('^[^/:]+/[^/:]+:[^/:]+$', config._attn_implementation):
                ...
                try:
                    ...
                    logger.warning(f"Could not find a kernel repository '{repo_id}' compatible with your devicein the hub: {e}. Using eager attention implementation instead.")
                    config._attn_implementation = 'eager'
                except AttributeError:
                    raise ValueError('the kernel function name or class specified in the attn_implementation argument is not valid.                                      Please check the documentation for the correct format,                                      and check that the kernel exports the class and the function correctly.')
            if not isinstance(config._attn_implementation, dict) and config._attn_implementation not in ['eager'] + ALL_ATTENTION_FUNCTIONS.valid_keys():
        ...
        for key in config.sub_configs.keys():
            ...
            curr_attn_implementation = requested_attn_implementation if not isinstance(requested_attn_implementation, dict) else requested_attn_implementation.get(key, None)
        ...
        if config._attn_implementation == 'flash_attention_2':
            ...
            if torch.version.hip is not None and config._attn_implementation == 'sdpa' and (torch.cuda.device_count() > 1) and (version.parse(torch.__version__) < version.parse('2.4.1')):
    ...
    @classmethod
    def _set_default_torch_dtype(cls: Any, dtype: torch.dtype) -> torch.dtype:
        """
    ...
    @property
    def base_model(self: Any) -> nn.Module:
        """
    ...
    def can_generate(cls: Any) -> bool:
        ...
        if hasattr(cls, 'prepare_inputs_for_generation'):
            logger.warning(f'''{cls.__name__} has generative capabilities, as `prepare_inputs_for_generation` is explicitly "\n                "defined. However, it doesn't directly inherit from `GenerationMixin`. From 👉v4.50👈 onwards, "\n                "`PreTrainedModel` will NOT inherit from `GenerationMixin`, and this model will lose the ability "\n                "to call `generate` and other related functions."\n                "\n  - If you're using `trust_remote_code=True`, you can get rid of this warning by loading the "\n                "model with an auto class. See https://huggingface.co/docs/transformers/en/model_doc/auto#auto-classes"\n                "\n  - If you are the owner of the model architecture code, please modify your model class such that "\n                "it inherits from `GenerationMixin` (after `PreTrainedModel`, otherwise you'll get an exception)."\n                "\n  - If you are not the owner of the model architecture class, please contact the model code owner "\n                "to update it."''')
        return False

    @classmethod
    def _check_and_enable_flash_attn_2(cls: Any, config: Any, torch_dtype: Optional[torch.dtype]=None, device_map: Optional[Union[str, Dict[str, int]]]=None, check_device_map: bool=True, hard_check_only: bool=False) -> PretrainedConfig:
        """
        ...
        if not cls._supports_flash_attn_2:
            raise ValueError(f'{cls.__name__} does not support Flash Attention 2.0 yet. Please request to add support where"\n                f" the model is hosted, on its model hub page: https://huggingface.co/{config._name_or_path}/discussions/new"\n                " or in the Transformers GitHub repo: https://github.com/huggingface/transformers/issues/new"')
        if not is_flash_attn_2_available():
            ...
            if torch.version.cuda:
                if flash_attention_version < version.parse('2.1.0'):
                    ...
                    raise ValueError(f'{preface} Flash Attention 2 is not available on CPU. Please make sure torch can access a CUDA device.')
                else:
        ...
        if torch_dtype is None:
            ...
            logger.warning_once(f""""Flash Attention 2.0 only supports torch.float16 and torch.bfloat16 dtypes, but"\n                f" the current dype in {cls.__name__} is {torch_dtype}. You should run training or inference using Automatic Mixed-Precision via the `with torch.autocast(device_type='torch_device'):` decorator,"\n                ' or load the model with the `torch_dtype` argument. Example: `model = AutoModel.from_pretrained("openai/whisper-tiny", attn_implementation="flash_attention_2", torch_dtype=torch.float16)`'""")
        if check_device_map and device_map is None and (torch.empty(0).device.type not in ['cuda', 'mlu']):
            if torch.cuda.is_available():
                logger.warning_once('You are attempting to use Flash Attention 2.0 with a model not initialized on GPU. Make sure to move the model to GPU"\n                    " after initializing it on CPU with `model.to(\'cuda\')`.')
            elif is_torch_mlu_available():
                logger.warning_once('You are attempting to use Flash Attention 2.0 with a model not initialized on MLU. Make sure to move the model to MLU"\n                    " after initializing it on CPU with `model.to(\'mlu\')`.')
            else:
                raise ValueError('You are attempting to use Flash Attention 2.0 with a model not initialized on GPU and with no GPU available. "\n                    "This is not supported yet. Please make sure to have access to a GPU and either initialise the model on a GPU by passing a device_map "\n                    "or initialising the model on CPU and then moving it to GPU.')
            ...
            raise ValueError('You are attempting to use Flash Attention 2.0 with a model dispatched on CPU or disk. This is not supported. Please make sure to "\n                "initialise the model on a GPU by passing a device_map that contains only GPU devices as keys.')
        if not hard_check_only:
            config._attn_implementation = 'flash_attention_2'
        return config

    @classmethod
    def _check_and_enable_sdpa(cls: Any, config: Any, hard_check_only: bool=False) -> PretrainedConfig:
        """
        ...
        if hard_check_only:
            if not cls._supports_sdpa:
                raise ValueError(f"""{cls.__name__} does not support an attention implementation through torch.nn.functional.scaled_dot_product_attention yet."\n                    " Please request the support for this architecture: https://github.com/huggingface/transformers/issues/28005. If you believe"\n                    ' this error is a bug, please open an issue in Transformers GitHub repository and load your model with the argument `attn_implementation="eager"` meanwhile. Example: `model = AutoModel.from_pretrained("openai/whisper-tiny", attn_implementation="eager")`'""")
    ...
    @classmethod
    def _check_and_enable_flex_attn(cls: Any, config: Any, hard_check_only: bool=False) -> PretrainedConfig:
        """
        ...
        if hard_check_only:
            if not cls._supports_flex_attn:
                raise ValueError(f"""{cls.__name__} does not support an attention implementation through torch's flex_attention."\n                    " Please request the support for this architecture: https://github.com/huggingface/transformers/issues/34809."\n                    " If you believe this error is a bug, please open an issue in Transformers GitHub repository"\n                    ' and load your model with the argument `attn_implementation="eager"` meanwhile.'\n                    ' Example: `model = AutoModel.from_pretrained("openai/whisper-tiny", attn_implementation="eager")`'""")
            if not is_torch_flex_attn_available():
                raise ImportError('PyTorch Flex Attention requirements in Transformers are not met. Please install torch>=2.5.0.')
    ...
    def enable_input_require_grads(self: Any) -> object:
        """
        Enables the gradients for the input embeddings. This is useful for fine-tuning adapter weights while keeping
        the model weights fixed.
        """

        def make_inputs_require_grads(module: Any, input: Any, output: Any) -> object:
            output.requires_grad_(True)
        self._require_grads_hook = self.get_input_embeddings().register_forward_hook(make_inputs_require_grads)

    def disable_input_require_grads(self: Any) -> object:
        """
        Removes the `_require_grads_hook`.
        """
        self._require_grads_hook.remove()

    def get_input_embeddings(self: Any) -> nn.Module:
        """
    ...
    def set_input_embeddings(self: Any, value: nn.Module) -> object:
        """
    ...
    def get_output_embeddings(self: Any) -> nn.Module:
        """
    ...
    def _initialize_weights(self: Any, module: Any) -> object:
        """
    ...
    @torch.no_grad()
    def initialize_weights(self: Any) -> object:
        """
        ...
        if not hasattr(torch.nn.Module, 'smart_apply'):
            ...
            def smart_apply(self: Any, fn: Any) -> object:
    ...
    def tie_weights(self: Any) -> object:
        """
    ...
    @staticmethod
    def _tie_encoder_decoder_weights(encoder: nn.Module, decoder: nn.Module, base_model_prefix: str, base_encoder_name: str) -> object:
        uninitialized_encoder_weights: List[str] = []
        tied_weights: List[str] = []
        if decoder.__class__ != encoder.__class__:
            logger.info(f'{decoder.__class__} and {encoder.__class__} are not equal. In this case make sure that all encoder"\n                " weights are correctly initialized."')

        def tie_encoder_to_decoder_recursively(decoder_pointer: nn.Module, encoder_pointer: nn.Module, module_name: str, base_encoder_name: str, uninitialized_encoder_weights: List[str], depth: Any=0, total_decoder_name: Any='', total_encoder_name: Any='') -> object:
            ...
            if len(decoder_modules) > 0:
                ...
                for name, module in decoder_modules.items():
                    if name.isdigit():
                        ...
                        raise ValueError('Max depth of recursive function `tie_encoder_to_decoder` reached. It seems that there is"\n                            " a circular dependency between two or more `nn.Modules` of your model.')
                    else:
    ...
    def _tie_or_clone_weights(self: Any, output_embeddings: Any, input_embeddings: Any) -> object:
    ...
    def _get_no_split_modules(self: Any, device_map: str) -> object:
        """
            ...
            if module.__class__.__name__ not in _no_split_modules:
                if isinstance(module, PreTrainedModel):
                    if module._no_split_modules is None:
                        raise ValueError(f'''{module.__class__.__name__} does not support `device_map='{device_map}'`. To implement support, the model "\n                            "class needs to implement the `_no_split_modules` attribute."''')
                    else:
                        _no_split_modules = _no_split_modules | set(module._no_split_modules)
                modules_to_check += list(module.children())
        return list(_no_split_modules)

    def resize_token_embeddings(self: Any, new_num_tokens: Optional[int]=None, pad_to_multiple_of: Optional[int]=None, mean_resizing: bool=True) -> nn.Embedding:
        """
    ...
    def _resize_token_embeddings(self: Any, new_num_tokens: Any, pad_to_multiple_of: Any=None, mean_resizing: Any=True) -> object:
        ...
        if pad_to_multiple_of is not None:
            if is_deepspeed_zero3_enabled() and (not is_quantized):
                ...
                new_num_tokens = new_embeddings.weight.shape[0]
        if self.get_output_embeddings() is not None and (not self.config.get_text_config(decoder=True).tie_word_embeddings):
    ...
    def _get_resized_embeddings(self: Any, old_embeddings: nn.Embedding, new_num_tokens: Optional[int]=None, pad_to_multiple_of: Optional[int]=None, mean_resizing: bool=True) -> nn.Embedding:
        """
        ...
        if pad_to_multiple_of is not None:
            ...
            logger.info(f'"You are resizing the embedding layer without providing a `pad_to_multiple_of` parameter. This means that the new embedding"\n                f" dimension will be {new_num_tokens}. This might induce some performance reduction as *Tensor Cores* will not be available."\n                " For more details about this, or help on choosing the correct value for resizing, refer to this guide:"\n                " https://docs.nvidia.com/deeplearning/performance/dl-performance-matrix-multiplication/index.html#requirements-tc"')
        if new_num_tokens is None:
        ...
        if new_num_tokens > old_num_tokens and (not mean_resizing):
            ...
            logger.warning_once('The new embeddings will be initialized from a multivariate normal distribution that has old embeddings\' mean and covariance. "\n                "As described in this article: https://nlp.stanford.edu/~johnhew/vocab-expansion.html. "\n                "To disable this, use `mean_resizing=False`')
            added_num_tokens = new_num_tokens - old_num_tokens
            if is_deepspeed_zero3_enabled() and (not is_quantized):
                import deepspeed
                with deepspeed.zero.GatheredParameters([old_embeddings.weight], modifier_rank=None):
                    self._init_added_embeddings_weights_with_mean(old_embeddings, new_embeddings, old_embedding_dim, old_num_tokens, added_num_tokens)
            else:
                self._init_added_embeddings_weights_with_mean(old_embeddings, new_embeddings, old_embedding_dim, old_num_tokens, added_num_tokens)
    ...
    def _get_resized_lm_head(self: Any, old_lm_head: nn.Linear, new_num_tokens: Optional[int]=None, transposed: Optional[bool]=False, mean_resizing: bool=True) -> nn.Linear:
        """
        ...
        if not isinstance(old_lm_head, nn.Linear):
            raise TypeError(f'Old language model head is of type {type(old_lm_head)}, which is not an instance of {nn.Linear}. You"\n                " should either use a different resize function or make sure that `old_lm_head` are an instance of"\n                f" {nn.Linear}.')
        ...
        if new_num_tokens > old_num_tokens and (not mean_resizing):
            ...
            logger.warning_once('The new lm_head weights will be initialized from a multivariate normal distribution that has old embeddings\' mean and covariance. "\n                "As described in this article: https://nlp.stanford.edu/~johnhew/vocab-expansion.html. "\n                "To disable this, use `mean_resizing=False`')
            added_num_tokens = new_num_tokens - old_num_tokens
            if is_deepspeed_zero3_enabled() and (not is_quantized):
                import deepspeed
    ...
    def _init_added_embeddings_weights_with_mean(self: Any, old_embeddings: Any, new_embeddings: Any, old_embedding_dim: Any, old_num_tokens: Any, added_num_tokens: Any) -> object:
        ...
        if is_covariance_psd:
            distribution = torch.distributions.multivariate_normal.MultivariateNormal(mean_embeddings, covariance_matrix=epsilon * covariance)
            new_embeddings.weight.data[-1 * added_num_tokens:, :] = distribution.sample(sample_shape=(added_num_tokens,)).to(old_embeddings.weight.dtype)
        else:
            new_embeddings.weight.data[-1 * added_num_tokens:, :] = mean_embeddings[None, :].repeat(added_num_tokens, 1).to(old_embeddings.weight.dtype)

    def _init_added_lm_head_weights_with_mean(self: Any, old_lm_head: Any, new_lm_head: Any, old_lm_head_dim: Any, old_num_tokens: Any, added_num_tokens: Any, transposed: Any=False) -> object:
        if transposed:
            new_lm_head.weight.data = new_lm_head.weight.data.T
            old_lm_head.weight.data = old_lm_head.weight.data.T
        self._init_added_embeddings_weights_with_mean(old_lm_head, new_lm_head, old_lm_head_dim, old_num_tokens, added_num_tokens)
        if transposed:
            new_lm_head.weight.data = new_lm_head.weight.data.T
            old_lm_head.weight.data = old_lm_head.weight.data.T

    def _init_added_lm_head_bias_with_mean(self: Any, old_lm_head: Any, new_lm_head: Any, added_num_tokens: Any) -> object:
        bias_mean = torch.mean(old_lm_head.bias.data, axis=0, dtype=torch.float32)
        bias_std = torch.std(old_lm_head.bias.data, axis=0).to(torch.float32)
        new_lm_head.bias.data[-1 * added_num_tokens:].normal_(mean=bias_mean, std=1e-09 * bias_std)

    def _copy_lm_head_original_to_resized(self: Any, new_lm_head: Any, old_lm_head: Any, num_tokens_to_copy: Any, transposed: Any, has_new_lm_head_bias: Any) -> object:
        if not transposed:
    ...
    def resize_position_embeddings(self: Any, new_num_position_embeddings: int) -> object:
        raise NotImplementedError(f'`resize_position_embeddings` is not implemented for {self.__class__}`. To implement it, you should "\n            f"overwrite this method in the class {self.__class__} in `modeling_{self.__class__.__module__}.py`')

    def get_position_embeddings(self: Any) -> Union[nn.Embedding, Tuple[nn.Embedding]]:
        raise NotImplementedError(f'`get_position_embeddings` is not implemented for {self.__class__}`. To implement it, you should "\n            f"overwrite this method in the class {self.__class__} in `modeling_{self.__class__.__module__}.py`')

    def init_weights(self: Any) -> object:
        """
    ...
    def prune_heads(self: Any, heads_to_prune: Dict[int, List[int]]) -> object:
        """
    ...
    def gradient_checkpointing_enable(self: Any, gradient_checkpointing_kwargs: Any=None) -> object:
        """
        ...
        if not _is_using_old_format:
            ...
            logger.warning('You are using an old version of the checkpointing format that is deprecated (We will also silently ignore `gradient_checkpointing_kwargs` in case you passed it)."\n                "Please update to the new format on your modeling file. To use the new format, you need to completely remove the definition of the method `_set_gradient_checkpointing` in your model.')
        if getattr(self, '_hf_peft_config_loaded', False):
            self.enable_input_require_grads()

    def _set_gradient_checkpointing(self: Any, enable: bool=True, gradient_checkpointing_func: Callable=checkpoint) -> object:
        ...
        if not is_gradient_checkpointing_set:
            raise ValueError(f'{self.__class__.__name__} is not compatible with gradient checkpointing. Make sure all the architecture support it by setting a boolean attribute"\n                " `gradient_checkpointing` to modules of the model that uses checkpointing."')

    def gradient_checkpointing_disable(self: Any) -> object:
        """
    ...
    @property
    def is_gradient_checkpointing(self: Any) -> bool:
        """
    ...
    def save_pretrained(self: Any, save_directory: Union[str, os.PathLike], is_main_process: bool=True, state_dict: Optional[dict]=None, save_function: Callable=torch.save, push_to_hub: bool=False, max_shard_size: Union[int, str]='5GB', safe_serialization: bool=True, variant: Optional[str]=None, token: Optional[Union[str, bool]]=None, save_peft_format: bool=True, **kwargs: Any) -> object:
        ...
        hf_quantizer = getattr(self, 'hf_quantizer', None)
        quantization_serializable = hf_quantizer is not None and isinstance(hf_quantizer, HfQuantizer) and hf_quantizer.is_serializable(safe_serialization=safe_serialization)
        if hf_quantizer is not None and (not _hf_peft_config_loaded) and (not quantization_serializable):
            raise ValueError(f'The model is quantized with {hf_quantizer.quantization_config.quant_method} and is not serializable - check out the warnings from"\n                " the logger on the traceback to understand the reason why the quantized model is not serializable."')
        ...
        if is_main_process:
            if not _hf_peft_config_loaded:
                ...
                if self.can_generate() and len(misplaced_generation_parameters) > 0:
                    warnings.warn(f'''"Moving the following attributes in the config to the generation config: "{misplaced_generation_parameters}. You are seeing this warning because you've set "\n                        "generation parameters in the model config, as opposed to in the generation config."''', UserWarning)
            ...
            if _hf_peft_config_loaded:
                ...
                if len(active_adapter) > 1:
                    raise ValueError('Multiple active adapters detected, saving multiple active adapters is not supported yet. You can save adapters separately one by one "\n                        "by iteratively calling `model.set_adapter(adapter_name)` then `model.save_pretrained(...)`')
        ...
        for shard_file, tensors in filename_to_tensors:
            ...
            for tensor in tensors:
                ...
                destroy([state_dict[tensor]])
                del state_dict[tensor]
            if module_map:
                if accelerate_version < version.parse('0.31'):
                    raise ImportError(f'You need accelerate version to be greater or equal than 0.31 to save models with offloaded parameters. Detected version {accelerate_version}. "\n                        f"Please upgrade accelerate with `pip install -U accelerate`')
        ...
        if index is None:
            ...
            with open(save_index_file, 'w', encoding='utf-8') as f:
                ...
                f.write(content)
            logger.info(f'The model is bigger than the maximum size per checkpoint ({max_shard_size}) and is going to be "\n                f"split in {len(state_dict_split.filename_to_tensors)} checkpoint shards. You can find where each parameters has been saved in the "\n                f"index located at {save_index_file}.')
        if push_to_hub:
    ...
    def push_to_hub(self: Any, *args: Any, **kwargs: Any) -> object:
    ...
    def get_memory_footprint(self: Any, return_buffers: Any=True) -> object:
        '''r"""
    ...
    def cuda(self: Any, *args: Any, **kwargs: Any) -> object:
        if getattr(self, 'quantization_method', None) == QuantizationMethod.HQQ:
            raise ValueError('`.cuda` is not supported for HQQ-quantized models.')
        if getattr(self, 'quantization_method', None) == QuantizationMethod.BITS_AND_BYTES:
            if getattr(self, 'is_loaded_in_8bit', False):
                raise ValueError('Calling `cuda()` is not supported for `8-bit` quantized models. "\n                    " Please use the model as it is, since the model has already been set to the correct devices.')
            elif version.parse(importlib.metadata.version('bitsandbytes')) < version.parse('0.43.2'):
                raise ValueError(f'"Calling `cuda()` is not supported for `4-bit` quantized models with the installed version of bitsandbytes. "\n                    f"The current device is `{self.device}`. If you intended to move the model, please install bitsandbytes >= 0.43.2.')
        else:
            return super().cuda(*args, **kwargs)

    @wraps(torch.nn.Module.to)
    def to(self: Any, *args: Any, **kwargs: Any) -> object:
        ...
        if getattr(self, 'quantization_method', None) == QuantizationMethod.BITS_AND_BYTES:
            if dtype_present_in_args:
                raise ValueError('You cannot cast a bitsandbytes model in a new `dtype`. Make sure to load the model using `from_pretrained` using the"\n                    " desired `dtype` by passing the correct `torch_dtype` argument.')
            if getattr(self, 'is_loaded_in_8bit', False):
                raise ValueError('`.to` is not supported for `8-bit` bitsandbytes models. Please use the model as it is, since the"\n                    " model has already been set to the correct devices and casted to the correct `dtype`.')
            elif version.parse(importlib.metadata.version('bitsandbytes')) < version.parse('0.43.2'):
                raise ValueError(f'"Calling `to()` is not supported for `4-bit` quantized models with the installed version of bitsandbytes. "\n                    f"The current device is `{self.device}`. If you intended to move the model, please install bitsandbytes >= 0.43.2.')
        elif getattr(self, 'quantization_method', None) == QuantizationMethod.GPTQ:
            if dtype_present_in_args:
                raise ValueError('You cannot cast a GPTQ model in a new `dtype`. Make sure to load the model using `from_pretrained` using the desired"\n                    " `dtype` by passing the correct `torch_dtype` argument.')
        return super().to(*args, **kwargs)

    def half(self: Any, *args: Any) -> object:
        if getattr(self, 'is_quantized', False):
            raise ValueError('`.half()` is not supported for quantized model. Please use the model as it is, since the"\n                " model has already been casted to the correct `dtype`.')
        else:
            return super().half(*args)

    def float(self: Any, *args: Any) -> object:
        if getattr(self, 'is_quantized', False):
            raise ValueError('`.float()` is not supported for quantized model. Please use the model as it is, since the"\n                " model has already been casted to the correct `dtype`.')
        else:
            return super().float(*args)

    @classmethod
    def get_init_context(cls: Any, is_quantized: bool, _is_ds_init_called: bool) -> object:
    ...
    @classmethod
    @restore_default_torch_dtype
    def from_pretrained(cls: Type[SpecificPreTrainedModelType], pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args: Any, config: Optional[Union[PretrainedConfig, str, os.PathLike]]=None, cache_dir: Optional[Union[str, os.PathLike]]=None, ignore_mismatched_sizes: bool=False, force_download: bool=False, local_files_only: bool=False, token: Optional[Union[str, bool]]=None, revision: str='main', use_safetensors: Optional[bool]=None, weights_only: bool=True, **kwargs: Any) -> SpecificPreTrainedModelType:
        '''r"""
        ...
        if state_dict is not None and (pretrained_model_name_or_path is not None or gguf_file is not None):
            raise ValueError('`state_dict` cannot be passed together with a model name or a `gguf_file`. Use one of the two loading strategies.')
        ...
        if is_peft_available():
            ...
            if _adapter_model_path is None:
                _adapter_model_path = find_adapter_config_file(pretrained_model_name_or_path, cache_dir=cache_dir, force_download=force_download, proxies=proxies, local_files_only=local_files_only, _commit_hash=commit_hash, **adapter_kwargs)
        ...
        if isinstance(device_map, torch.device):
            ...
            try:
                ...
                raise ValueError(f""""When passing device_map as a string, the value needs to be a device name (e.g. cpu, cuda:0) or "\n                    f"'auto', 'balanced', 'balanced_low_0', 'sequential' but found {device_map}.""")
        ...
        if device_map is not None:
            ...
            if not is_accelerate_available():
                raise ValueError('Using a `device_map`, `tp_plan`, `torch.device` context manager or setting `torch.set_default_device(device)` "\n                        "requires `accelerate`. You can install it with `pip install accelerate`')
        if load_in_4bit or load_in_8bit:
            if quantization_config is not None:
                raise ValueError('You can\'t pass `load_in_4bit`or `load_in_8bit` as a kwarg when passing "\n                    "`quantization_config` argument at the same time.')
            ...
            logger.warning('The `load_in_4bit` and `load_in_8bit` arguments are deprecated and will be removed in the future versions. "\n                "Please, pass a `BitsAndBytesConfig` object in `quantization_config` argument instead.')
        ...
        if transformers_explicit_filename is not None:
            if not transformers_explicit_filename.endswith('.safetensors') and (not transformers_explicit_filename.endswith('.safetensors.index.json')):
                raise ValueError(f'"The transformers file in the config seems to be incorrect: it is neither a safetensors file "\n                    "(*.safetensors) nor a safetensors index file (*.safetensors.index.json): "{transformers_explicit_filename}')
        ...
        if pre_quantized or quantization_config is not None:
            ...
            hf_quantizer = None
        if hf_quantizer is not None:
            hf_quantizer.validate_environment(torch_dtype=torch_dtype, from_tf=from_tf, from_flax=from_flax, device_map=device_map, weights_only=weights_only)
        ...
        if gguf_file and device_map is not None and (isinstance(device_map, dict) and 'disk' in device_map.values() or 'disk' in device_map):
            raise RuntimeError('One or more modules is configured to be mapped to disk. Disk offload is not supported for models "\n                "loaded from GGUF files.')
        checkpoint_files, sharded_metadata = _get_resolved_checkpoint_files(pretrained_model_name_or_path=pretrained_model_name_or_path, subfolder=subfolder, variant=variant, gguf_file=gguf_file, from_tf=from_tf, from_flax=from_flax, use_safetensors=use_safetensors, cache_dir=cache_dir, force_download=force_download, proxies=proxies, local_files_only=local_files_only, token=token, user_agent=user_agent, revision=revision, commit_hash=commit_hash, transformers_explicit_filename=transformers_explicit_filename)
        is_sharded = sharded_metadata is not None
        is_quantized = hf_quantizer is not None
        is_from_file = pretrained_model_name_or_path is not None or gguf_file is not None
        if is_safetensors_available() and is_from_file and (not is_sharded) and checkpoint_files[0].endswith('.safetensors'):
        ...
        if from_pt:
            if gguf_file:
                ...
                state_dict = load_gguf_checkpoint(checkpoint_files[0], return_tensors=True, model_to_load=dummy_model)['tensors']
            config, torch_dtype, dtype_orig = _get_torch_dtype(cls, torch_dtype, checkpoint_files, config, sharded_metadata, state_dict, weights_only)
        ...
        if model._keep_in_fp32_modules is not None and (torch_dtype == torch.float16 or getattr(hf_quantizer, 'use_keep_in_fp32_modules', False)):
            keep_in_fp32_regex = re.compile('|'.join([f'((^|\\.){module}($|\\.))' for module in model._keep_in_fp32_modules]))
        if hf_quantizer is not None:
            hf_quantizer.preprocess_model(model=model, device_map=device_map, keep_in_fp32_modules=model._keep_in_fp32_modules, config=config)
            original_dtype = torch_dtype if torch_dtype is not None else torch.get_default_dtype()

            def _assign_original_dtype(module: Any) -> object:
    ...
    def _get_key_renaming_mapping(self: Any, checkpoint_keys: List[str], key_mapping: Optional[Dict[str, str]]=None, loading_base_model_from_task_state_dict: bool=False, loading_task_model_from_base_state_dict: bool=False) -> object:
        """
    ...
    @staticmethod
    def _fix_state_dict_key_on_save(key: Any) -> Tuple[str, bool]:
        """
    ...
    def _fix_state_dict_keys_on_save(self: Any, state_dict: Any) -> object:
        """
    ...
    @classmethod
    def _load_pretrained_model(cls: Any, model: 'PreTrainedModel', state_dict: Optional[Dict], checkpoint_files: Optional[List[str]], pretrained_model_name_or_path: Optional[str], ignore_mismatched_sizes: bool=False, sharded_metadata: Optional[Dict]=None, device_map: Optional[Dict]=None, disk_offload_folder: Optional[str]=None, offload_state_dict: Optional[bool]=None, dtype: Optional[torch.dtype]=None, hf_quantizer: Optional[HfQuantizer]=None, keep_in_fp32_regex: Optional[re.Pattern]=None, device_mesh: Optional['torch.distributed.device_mesh.DeviceMesh']=None, key_mapping: Optional[Dict[str, str]]=None, weights_only: bool=True) -> object:
        is_quantized = hf_quantizer is not None
        ...
        if sharded_metadata is not None:
            ...
            original_checkpoint_keys = list(load_state_dict(checkpoint_files[0], map_location='meta', weights_only=weights_only).keys())
        prefix = model.base_model_prefix
        _prefix = f'{prefix}.'
        ...
        missing_keys, unexpected_keys = _find_missing_and_unexpected_keys(cls, model, original_checkpoint_keys, checkpoint_keys, loading_base_model_from_task_state_dict, hf_quantizer, device_map)
        ...
        if loading_task_model_from_base_state_dict:
            ...
            if any((key in task_specific_expected_keys and key not in base_model_expected_keys for key in checkpoint_keys)):
                raise ValueError('The state dictionary of the model you are trying to load is corrupted. Are you sure it was "\n                    "properly saved?')
        ...
        if device_map is not None and 'disk' in device_map.values():
            ...
            if is_offloaded_safetensors:
                ...
                disk_offload_index = {name: {'safetensors_file': file, 'weight_name': reverse_key_renaming_mapping[name], 'dtype': str_dtype} for name, file in weight_map.items() if param_device_map[name] == 'disk'}
            else:
                disk_offload_index = {}
        ...
        for shard_file in checkpoint_files:
            ...
            if shard_file.endswith('.safetensors') and (not is_hqq_or_bnb) and (not (is_deepspeed_zero3_enabled() and (not is_quantized))):
                map_location = 'meta'
            elif device_map is not None and hf_quantizer is not None and (hf_quantizer.quantization_config.quant_method == QuantizationMethod.TORCHAO) and (hf_quantizer.quantization_config.quant_type in ['int4_weight_only', 'autoquant'] or isinstance(hf_quantizer.quantization_config.quant_type, Int4WeightOnlyConfig)):
            ...
            if loading_task_model_from_base_state_dict:
                ...
                if not is_offloaded_safetensors:
                    for weight_name in disk_offload_index:
                        shutil.move(os.path.join(disk_offload_folder, f'{weight_name}.dat'), os.path.join(disk_offload_folder, f'{prefix}.{weight_name}.dat'))
        ...
        if device_mesh is not None:
            ...
            if loading_task_model_from_base_state_dict:
                parameters_to_initialize = {name: param for name, param in model.named_parameters() if not name.startswith(prefix)}
                for name, param in parameters_to_initialize.items():
                    if param.device.type == 'meta':
                        continue
                    to_contiguous, casting_dtype = _infer_parameter_dtype(model, name, param, keep_in_fp32_regex)
                    shard_and_distribute_module(model, param.to(tp_device), param, name, casting_dtype, to_contiguous, device_mesh.get_local_rank(), device_mesh)
        if len(error_msgs) > 0:
            error_msg = '\n\t'.join(error_msgs)
        ...
        if len(mismatched_keys) > 0:
            mismatched_warning = '\n'.join([f'- {key}: found shape {shape1} in the checkpoint and {shape2} in the model instantiated' for key, (shape1, shape2) in zip(mismatched_keys, mismatched_shapes)])
            logger.warning(f'Some weights of {model.__class__.__name__} were not initialized from the model checkpoint at"\n                f" {pretrained_model_name_or_path} and are newly initialized because the shapes did not"\n                f" match:\n{mismatched_warning}\nYou should probably TRAIN this model on a down-stream task to be able"\n                " to use it for predictions and inference."')
        return (model, missing_keys, unexpected_keys, mismatched_keys, disk_offload_index, error_msgs)

    @classmethod
    def _load_from_tf(cls: Any, model: Any, config: Any, checkpoint_files: Any) -> object:
        if checkpoint_files[0].endswith('.index'):
            ...
            try:
                ...
                model, loading_info = load_tf2_checkpoint_in_pytorch_model(model, checkpoint_files[0], allow_missing_keys=True, output_loading_info=True)
            except ImportError:
                logger.error('Loading a TensorFlow model in PyTorch, requires both PyTorch and TensorFlow to be installed."\n                    " Please see https://pytorch.org/ and https://www.tensorflow.org/install/ for installation"\n                    " instructions.')
                raise
        return (model, loading_info)

    @classmethod
    def _load_from_flax(cls: Any, model: Any, checkpoint_files: Any) -> object:
        try:
            from .modeling_flax_pytorch_utils import load_flax_checkpoint_in_pytorch_model
            model = load_flax_checkpoint_in_pytorch_model(model, checkpoint_files[0])
        except ImportError:
            logger.error('Loading a Flax model in PyTorch, requires both PyTorch and Flax to be installed. Please see"\n                " https://pytorch.org/ and https://flax.readthedocs.io/en/latest/installation.html for"\n                " installation instructions.')
            raise
        return model

    def retrieve_modules_from_names(self: Any, names: Any, add_prefix: Any=False, remove_prefix: Any=False) -> object:
    ...
    @classmethod
    def register_for_auto_class(cls: Any, auto_class: Any='AutoModel') -> object:
        """
    ...
    def to_bettertransformer(self: Any) -> 'PreTrainedModel':
        """
    ...
    def reverse_bettertransformer(self: Any) -> object:
        """
    ...
    def warn_if_padding_and_no_attention_mask(self: Any, input_ids: Any, attention_mask: Any) -> object:
        """
        ...
        if self.config.pad_token_id in input_ids[:, [-1, 0]]:
            warn_string = 'We strongly recommend passing in an `attention_mask` since your input_ids may be padded. See "\n                "https://huggingface.co/docs/transformers/troubleshooting"\n                "#incorrect-output-when-padding-tokens-arent-masked.'
            ...
            __gm_se_arg_1 = warn_string
            __jac_se_emit__(logger.warning_once, (__gm_se_arg_1.clone() if hasattr(__gm_se_arg_1, 'clone') else __gm_se_arg_1,), {})

    @property
    def supports_tp_plan(self: Any) -> object:
        """
    ...
    @property
    def supports_pp_plan(self: Any) -> object:
    ...
    @property
    def loss_function(self: Any) -> object:
        if hasattr(self, '_loss_function'):
            return self._loss_function
        loss_type = getattr(self, 'loss_type', None)
        if loss_type is None or loss_type not in LOSS_MAPPING:
            logger.warning_once(f'`loss_type={loss_type}` was set in the config but it is unrecognised."\n                f"Using the default loss: `ForCausalLMLoss`.')
            loss_type = 'ForCausalLM'
        return LOSS_MAPPING[loss_type]

    @loss_function.setter
    def loss_function(self: Any, value: Any) -> object:
        self._loss_function = value

    def get_compiled_call(self: Any, compile_config: Optional[CompileConfig]) -> Callable:
        ...
        if not hasattr(self, '_compiled_call') or getattr(self, '_last_compile_config', default_config) != compile_config:
            self._last_compile_config = compile_config
            self._compiled_call = torch.compile(self.__call__, **compile_config.to_dict())
        return self._compiled_call

    @classmethod
    def is_backend_compatible(cls: Any) -> object:
        return cls._supports_attention_backend

    def _move_missing_keys_from_meta_to_cpu(self: Any, missing_keys: List[str], unexpected_keys: List[str], dtype: Optional[torch.dtype], hf_quantizer: Optional[HfQuantizer]) -> 'PreTrainedModel':
        ...
        for key in missing_keys:
            ...
            if param.device == torch.device('meta'):
                value = torch.empty_like(param, dtype=dtype, device='cpu')
                if not is_quantized or getattr(hf_quantizer, 'requires_parameters_quantization', False) or (not hf_quantizer.check_quantized_param(self, param_value=value, param_name=key, state_dict={})):
                    _load_parameter_into_model(self, key, value)
                else:
                    hf_quantizer.create_quantized_param(self, value, key, 'cpu', model_state_dict, unexpected_keys)

    def _initialize_missing_keys(self: Any, loaded_keys: List[str], ignore_mismatched_sizes: bool, is_quantized: bool) -> 'PreTrainedModel':
        ...
        if not ignore_mismatched_sizes:
            ...
            if hasattr(self.config.get_text_config(decoder=True), 'tie_word_embeddings') and self.config.get_text_config(decoder=True).tie_word_embeddings:
                output_embeddings = self.get_output_embeddings()
                if output_embeddings is not None:
    ...
    def get_parameter_or_buffer(self: Any, target: str) -> object:
        """
...
class PoolerStartLogits(nn.Module):
    ...
    def __init__(self: Any, config: PretrainedConfig) -> object:
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, 1)
        logger.warning_once('[DEPRECATION WARNING] `PoolerStartLogits` is deprecated and will be removed in v4.53. "\n            "Please use model-specific class, e.g. `XLMPoolerStartLogits`.')

    def forward(self: Any, hidden_states: torch.FloatTensor, p_mask: Optional[torch.FloatTensor]=None) -> torch.FloatTensor:
        """
        Args:
...
class PoolerEndLogits(nn.Module):
    ...
    def __init__(self: Any, config: PretrainedConfig) -> object:
        ...
        logger.warning_once('[DEPRECATION WARNING] `PoolerEndLogits` is deprecated and will be removed in v4.53. "\n            "Please use model-specific class, e.g. `XLMPoolerEndLogits`.')

    def forward(self: Any, hidden_states: torch.FloatTensor, start_states: Optional[torch.FloatTensor]=None, start_positions: Optional[torch.LongTensor]=None, p_mask: Optional[torch.FloatTensor]=None) -> torch.FloatTensor:
        """
        Args:
            hidden_states (`torch.FloatTensor` of shape `(batch_size, seq_len, hidden_size)`):
                The final hidden states of the model.
...
class PoolerAnswerClass(nn.Module):
    ...
    def __init__(self: Any, config: Any) -> object:
        ...
        logger.warning_once('[DEPRECATION WARNING] `PoolerAnswerClass` is deprecated and will be removed in v4.53. "\n            "Please use model-specific class, e.g. `XLMPoolerAnswerClass`.')

    def forward(self: Any, hidden_states: torch.FloatTensor, start_states: Optional[torch.FloatTensor]=None, start_positions: Optional[torch.LongTensor]=None, cls_index: Optional[torch.LongTensor]=None) -> torch.FloatTensor:
        """
        Args:
            hidden_states (`torch.FloatTensor` of shape `(batch_size, seq_len, hidden_size)`):
                The final hidden states of the model.
...
class SquadHeadOutput(ModelOutput):
    ...
    def __post_init__(self: Any) -> object:
        logger.warning_once('[DEPRECATION WARNING] `SquadHeadOutput` is deprecated and will be removed in v4.53. "\n            "Please use model-specific class, e.g. `XLMSquadHeadOutput`.')
...
class SQuADHead(nn.Module):
    ...
    def __init__(self: Any, config: Any) -> object:
        ...
        logger.warning_once('[DEPRECATION WARNING] `SQuADHead` is deprecated and will be removed in v4.53. "\n            "Please use model-specific class, e.g. `XLMSQuADHead`.')

    @replace_return_docstrings(output_type=SquadHeadOutput, config_class=PretrainedConfig)
    def forward(self: Any, hidden_states: torch.FloatTensor, start_positions: Optional[torch.LongTensor]=None, end_positions: Optional[torch.LongTensor]=None, cls_index: Optional[torch.LongTensor]=None, is_impossible: Optional[torch.LongTensor]=None, p_mask: Optional[torch.FloatTensor]=None, return_dict: bool=False) -> Union[SquadHeadOutput, Tuple[torch.FloatTensor]]:
        """
        Args:
            hidden_states (`torch.FloatTensor` of shape `(batch_size, seq_len, hidden_size)`):
...
class SequenceSummary(nn.Module):
    ...
    def __init__(self: Any, config: PretrainedConfig) -> object:
        ...
        logger.warning_once('[DEPRECATION WARNING] `SequenceSummary` is deprecated and will be removed in v4.53. "\n            "Please use model-specific class, e.g. `XLMSequenceSummary`.')

    def forward(self: Any, hidden_states: torch.FloatTensor, cls_index: Optional[torch.LongTensor]=None) -> torch.FloatTensor:
        """
        Compute a single vector summary of a sequence hidden states.
...
def expand_device_map(device_map: Any, param_names: Any) -> object:
    """
...
def caching_allocator_warmup(model: PreTrainedModel, expanded_device_map: Dict, hf_quantizer: Optional[HfQuantizer]) -> object:
    ...
    if not len(accelerator_device_map):
        return
    tp_plan_regex = re.compile('|'.join([re.escape(plan) for plan in model._tp_plan])) if _torch_distributed_available and torch.distributed.is_initialized() else None
    total_byte_count = defaultdict(lambda: 0)
    for param_name, device in accelerator_device_map.items():
        param = model.get_parameter_or_buffer(param_name)
        param_byte_count = param.numel() * param.element_size()
        if tp_plan_regex is not None:
            generic_name = re.sub('\\.\\d+\\.', '.*.', param_name)
...
def get_disk_only_shard_files(device_map: Any, weight_map: Any) -> object:
    """
...
class AttentionInterface(MutableMapping):
    ...
    """
    _global_mapping = {'flash_attention_2': flash_attention_forward, 'flex_attention': flex_attention_forward, 'sdpa': sdpa_attention_forward}

    def __init__(self: Any) -> object:
        self._local_mapping = {}

    def __getitem__(self: Any, key: Any) -> object:
        if key in self._local_mapping:
            return self._local_mapping[key]
        return self._global_mapping[key]

    def __setitem__(self: Any, key: Any, value: Any) -> object:
        self._local_mapping.update({key: value})

    def __delitem__(self: Any, key: Any) -> object:
        destroy([self._local_mapping[key]])
        del self._local_mapping[key]

    def __iter__(self: Any) -> object:
        return iter({**self._global_mapping, **self._local_mapping})

    def __len__(self: Any) -> object:
        return len(self._global_mapping.keys() | self._local_mapping.keys())

    @classmethod
    def register(cls: Any, key: str, value: Callable) -> object:
        cls._global_mapping.update({key: value})

    def valid_keys(self: Any) -> List[str]:
