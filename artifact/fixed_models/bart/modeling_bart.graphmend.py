...
class BartLearnedPositionalEmbedding(nn.Embedding):
    ...
    def forward(self: Any, input_ids: torch.Tensor, past_key_values_length: int=0, position_ids: torch.Tensor=None) -> object:
...
class BartAttention(nn.Module):
    ...
    def __init__(self: Any, embed_dim: int, num_heads: int, dropout: float=0.0, is_decoder: bool=False, bias: bool=True, is_causal: bool=False, config: Optional[BartConfig]=None, layer_idx: Optional[int]=None) -> object:
        super().__init__()
...
class BartFlashAttention2(BartAttention):
    ...
    def forward(self: Any, hidden_states: torch.Tensor, key_value_states: Optional[torch.Tensor]=None, past_key_value: Optional[Cache]=None, attention_mask: Optional[torch.Tensor]=None, layer_head_mask: Optional[torch.Tensor]=None, output_attentions: bool=False, cache_position: Optional[torch.Tensor]=None) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        if output_attentions:
            raise ValueError('BartSdpaAttention2 attention does not support `output_attentions`. "\n                "Use the argument `attn_implementation=\'eager\'` when loading the model.')
        is_cross_attention = key_value_states is not None
        ...
        attn_output = _flash_attention_forward(query_states, key_states, value_states, attention_mask, q_len, dropout=self.dropout if self.training else 0.0, is_causal=self.is_causal, use_top_left_mask=self._flash_attn_uses_top_left_mask)
...
class BartSdpaAttention(BartAttention):
    ...
    def forward(self: Any, hidden_states: torch.Tensor, key_value_states: Optional[torch.Tensor]=None, past_key_value: Optional[Cache]=None, attention_mask: Optional[torch.Tensor]=None, layer_head_mask: Optional[torch.Tensor]=None, output_attentions: bool=False, cache_position: Optional[torch.Tensor]=None) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        """Input shape: Batch x Time x Channel"""
        if output_attentions:
            __jac_log_emit__(_gm_log_slot_1, ('BartModel is using BartSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True` . Falling back to the manual attention"\n                \' implementation, but specifying the manual implementation will be required from Transformers version v5.0.0 onwards. This warning can be removed using the argument `attn_implementation="eager"` when loading the model.',), {})
...
class BartEncoderLayer(nn.Module):
    ...
    def __init__(self: Any, config: BartConfig, layer_idx: Optional[int]=None) -> object:
        super().__init__()
        self.embed_dim = config.d_model
        self.self_attn = BART_ATTENTION_CLASSES[config._attn_implementation](embed_dim=self.embed_dim, num_heads=config.encoder_attention_heads, dropout=config.attention_dropout, config=config, layer_idx=layer_idx)
...
class BartDecoderLayer(nn.Module):
    ...
    def __init__(self: Any, config: BartConfig, layer_idx: Optional[int]=None) -> object:
        super().__init__()
        self.embed_dim = config.d_model
        self.self_attn = BART_ATTENTION_CLASSES[config._attn_implementation](embed_dim=self.embed_dim, num_heads=config.decoder_attention_heads, dropout=config.attention_dropout, is_decoder=True, is_causal=True, config=config, layer_idx=layer_idx)
        self.dropout = config.dropout
        self.activation_fn = ACT2FN[config.activation_function]
        self.activation_dropout = config.activation_dropout
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = BART_ATTENTION_CLASSES[config._attn_implementation](self.embed_dim, config.decoder_attention_heads, dropout=config.attention_dropout, is_decoder=True, config=config, layer_idx=layer_idx)
...
class BartClassificationHead(nn.Module):
    ...
    def __init__(self: Any, input_dim: int, inner_dim: int, num_classes: int, pooler_dropout: float) -> object:
        super().__init__()
...
class PretrainedBartModel(BartPreTrainedModel):
    ...
    def __init_subclass__(self: Any) -> object:
...
class BartEncoder(BartPreTrainedModel):
    ...
    def __init__(self: Any, config: BartConfig, embed_tokens: Optional[nn.Embedding]=None) -> object:
...
class BartDecoder(BartPreTrainedModel):
    ...
    def forward(self: Any, input_ids: Optional[torch.LongTensor]=None, attention_mask: Optional[torch.Tensor]=None, encoder_hidden_states: Optional[torch.FloatTensor]=None, encoder_attention_mask: Optional[torch.LongTensor]=None, head_mask: Optional[torch.Tensor]=None, cross_attn_head_mask: Optional[torch.Tensor]=None, past_key_values: Optional[List[torch.FloatTensor]]=None, inputs_embeds: Optional[torch.FloatTensor]=None, use_cache: Optional[bool]=None, output_attentions: Optional[bool]=None, output_hidden_states: Optional[bool]=None, return_dict: Optional[bool]=None, cache_position: Optional[torch.LongTensor]=None) -> Union[Tuple, BaseModelOutputWithPastAndCrossAttentions]:
        ...
        if use_cache and (not isinstance(past_key_values, Cache)):
            ...
            __jac_log_emit__(_gm_log_slot_3, ('Passing a tuple of `past_key_values` is deprecated and will be removed in Transformers v4.58.0. "\n                "You should pass an instance of `EncoderDecoderCache` instead, e.g. "\n                "`past_key_values=EncoderDecoderCache.from_legacy_cache(past_key_values)`.',), {})
            past_key_values = EncoderDecoderCache.from_legacy_cache(past_key_values)
        ...
        if encoder_hidden_states is not None and encoder_attention_mask is not None:
            if self._use_flash_attention_2:
                ...
                encoder_attention_mask = _prepare_4d_attention_mask_for_sdpa(encoder_attention_mask, inputs_embeds.dtype, tgt_len=seq_length)
            else:
                encoder_attention_mask = _prepare_4d_attention_mask(encoder_attention_mask, inputs_embeds.dtype, tgt_len=seq_length)
...
class BartModel(BartPreTrainedModel):
    ...
    def __init__(self: Any, config: BartConfig) -> object:
        super().__init__(config)
    ...
    def _tie_weights(self: Any) -> object:
        if self.config.tie_word_embeddings:
            if self.shared.weight.device == torch.device('meta') and self.decoder.embed_tokens.weight.device != torch.device('meta'):
...
class BartForConditionalGeneration(BartPreTrainedModel, GenerationMixin):
    ...
    def resize_token_embeddings(self: Any, new_num_tokens: int, pad_to_multiple_of: Optional[int]=None, mean_resizing: bool=True) -> nn.Embedding:
    ...
    def forward(self: Any, input_ids: Optional[torch.LongTensor]=None, attention_mask: Optional[torch.Tensor]=None, decoder_input_ids: Optional[torch.LongTensor]=None, decoder_attention_mask: Optional[torch.LongTensor]=None, head_mask: Optional[torch.Tensor]=None, decoder_head_mask: Optional[torch.Tensor]=None, cross_attn_head_mask: Optional[torch.Tensor]=None, encoder_outputs: Optional[List[torch.FloatTensor]]=None, past_key_values: Optional[List[torch.FloatTensor]]=None, inputs_embeds: Optional[torch.FloatTensor]=None, decoder_inputs_embeds: Optional[torch.FloatTensor]=None, labels: Optional[torch.LongTensor]=None, use_cache: Optional[bool]=None, output_attentions: Optional[bool]=None, output_hidden_states: Optional[bool]=None, return_dict: Optional[bool]=None, cache_position: Optional[torch.LongTensor]=None) -> Union[Tuple, Seq2SeqLMOutput]:
        ...
        if labels is not None:
            if use_cache:
                __jac_log_emit__(_gm_log_slot_4, ('The `use_cache` argument is changed to `False` since `labels` is provided.',), {})
            use_cache = False
            if decoder_input_ids is None and decoder_inputs_embeds is None:
...
_gm_log_slot_4 = __jac_log_register__(logger.warning)

@auto_docstring(custom_intro='\n    Bart model with a sequence classification/head on top (a linear layer on top of the pooled output) e.g. for GLUE\n    tasks.\n    ')
class BartForSequenceClassification(BartPreTrainedModel):
    _tied_weights_keys = ['encoder.embed_tokens.weight', 'decoder.embed_tokens.weight']

    def __init__(self: Any, config: BartConfig, **kwargs: Any) -> object:
...
class BartDecoderWrapper(BartPreTrainedModel):
    ...
    def forward(self: Any, *args: Any, **kwargs: Any) -> object:
...
class BartForCausalLM(BartPreTrainedModel, GenerationMixin):
    ...
    def set_decoder(self: Any, decoder: Any) -> object:
    ...
    def _reorder_cache(past_key_values: Any, beam_idx: Any) -> object:
        ...
        return reordered_past
__all__ = ['BartForCausalLM', 'BartForConditionalGeneration', 'BartForQuestionAnswering', 'BartForSequenceClassification', 'BartModel', 'BartPreTrainedModel', 'BartPretrainedModel', 'PretrainedBartModel']