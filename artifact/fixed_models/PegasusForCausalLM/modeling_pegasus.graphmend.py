...
class PegasusAttention(nn.Module):
    ...
    def __init__(self: Any, embed_dim: int, num_heads: int, dropout: float=0.0, is_decoder: bool=False, bias: bool=True, is_causal: bool=False, config: Optional[PegasusConfig]=None, layer_idx: Optional[int]=None) -> object:
        super().__init__()
...
class PegasusEncoderLayer(nn.Module):
    ...
    def __init__(self: Any, config: PegasusConfig) -> object:
        super().__init__()
...
class PegasusDecoderLayer(nn.Module):
    ...
    def __init__(self: Any, config: PegasusConfig, layer_idx: Optional[int]=None) -> object:
        super().__init__()
        self.embed_dim = config.d_model
        self.self_attn = PEGASUS_ATTENTION_CLASSES[config._attn_implementation](embed_dim=self.embed_dim, num_heads=config.decoder_attention_heads, dropout=config.attention_dropout, is_decoder=True, is_causal=True, config=config, layer_idx=layer_idx)
        self.dropout = config.dropout
        self.activation_fn = ACT2FN[config.activation_function]
        self.activation_dropout = config.activation_dropout
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = PEGASUS_ATTENTION_CLASSES[config._attn_implementation](self.embed_dim, config.decoder_attention_heads, dropout=config.attention_dropout, is_decoder=True, config=config, layer_idx=layer_idx)
...
class PegasusEncoder(PegasusPreTrainedModel):
    ...
    def __init__(self: Any, config: PegasusConfig, embed_tokens: Optional[nn.Embedding]=None) -> object:
    ...
    def get_position_embeddings(self: Any) -> nn.Embedding:
        """
...
class PegasusModel(PegasusPreTrainedModel):
    ...
    def get_position_embeddings(self: Any) -> Tuple[nn.Embedding]:
        """
        Returns the position embeddings matrix
        """
