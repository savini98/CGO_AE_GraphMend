...
def _xavier_init(conv: nn.Module) -> object:
...
class SSDHead(nn.Module):
    ...
    def __init__(self: Any, in_channels: List[int], num_anchors: List[int], num_classes: int) -> object:
        super().__init__()
...
class SSDScoringHead(nn.Module):
    ...
    def __init__(self: Any, module_list: nn.ModuleList, num_columns: int) -> object:
        super().__init__()
        self.module_list = module_list
        self.num_columns = num_columns

    def _get_result_from_module_list(self: Any, x: Tensor, idx: int) -> Tensor:
        """
...
class SSDRegressionHead(SSDScoringHead):
    ...
    def __init__(self: Any, in_channels: List[int], num_anchors: List[int]) -> object:
    ...
    def forward(self: Any, images: List[Tensor], targets: Optional[List[Dict[str, Tensor]]]=None) -> Tuple[Dict[str, Tensor], List[Dict[str, Tensor]]]:
        ...
        if torch.jit.is_scripting():
            if not self._has_warned:
                __jac_log_emit__(_gm_log_slot_0, ('SSD always returns a (Losses, Detections) tuple in scripting',), {})
                self._has_warned = True
            return (losses, detections)
        return self.eager_outputs(losses, detections)

    def postprocess_detections(self: Any, head_outputs: Dict[str, Tensor], image_anchors: List[Tensor], image_shapes: List[Tuple[int, int]]) -> List[Dict[str, Tensor]]:
        bbox_regression = head_outputs['bbox_regression']
...
class SSDFeatureExtractorVGG(nn.Module):
    ...
    def __init__(self: Any, backbone: nn.Module, highres: bool) -> object:
        super().__init__()
    ...
    def forward(self: Any, x: Tensor) -> Dict[str, Tensor]:
        x = self.features(x)
...
def _vgg_extractor(backbone: VGG, highres: bool, trainable_layers: int) -> object:
