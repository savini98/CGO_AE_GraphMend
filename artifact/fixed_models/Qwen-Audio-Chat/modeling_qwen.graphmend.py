...
from .qwen_generation_utils import HistoryType, make_context, decode_tokens, get_stop_words_ids, StopWordsLogitsProcessor
...
def _import_flash_attn() -> object:
    global apply_rotary_emb_func, rms_norm, flash_attn_unpadded_func
    try:
        from flash_attn.layers.rotary import apply_rotary_emb_func as __apply_rotary_emb_func
        apply_rotary_emb_func = __apply_rotary_emb_func
    except ImportError:
        logger.warn('Warning: import flash_attn rotary fail, please install FlashAttention rotary to get higher efficiency "\n            "https://github.com/Dao-AILab/flash-attention/tree/main/csrc/rotary')
    try:
        from flash_attn.ops.rms_norm import rms_norm as __rms_norm
        rms_norm = __rms_norm
    except ImportError:
        logger.warn('Warning: import flash_attn rms_norm fail, please install FlashAttention layer_norm to get higher efficiency "\n            "https://github.com/Dao-AILab/flash-attention/tree/main/csrc/layer_norm')
    try:
        import flash_attn
        ...
        logger.warn('Warning: import flash_attn fail, please install FlashAttention to get higher efficiency "\n            "https://github.com/Dao-AILab/flash-attention')

def quantize_cache_v(fdata: Any, bits: Any, qmax: Any, qmin: Any) -> object:
    qtype = torch.uint8
    device = fdata.device
    shape = fdata.shape
...
def dequantize_cache_torch(qdata: Any, scale: Any, zero: Any) -> object:
    data = scale * (qdata - zero)
    return data

class FlashSelfAttention(torch.nn.Module):

    def __init__(self: Any, causal: Any=False, softmax_scale: Any=None, attention_dropout: Any=0.0) -> object:
        super().__init__()
    ...
    def unpad_input(self: Any, hidden_states: Any, attention_mask: Any) -> object:
    ...
    def pad_input(self: Any, hidden_states: Any, indices: Any, batch: Any, seqlen: Any) -> object:
        output = torch.zeros(batch * seqlen, *hidden_states.shape[1:], device=hidden_states.device, dtype=hidden_states.dtype)
        output[indices] = hidden_states
        return rearrange(output, '(b s) ... -> b s ...', b=batch)

    def forward(self: Any, q: Any, k: Any, v: Any, attention_mask: Any=None) -> object:
...
class QWenAttention(nn.Module):
    ...
    def _attn(self: Any, query: Any, key: Any, value: Any, registered_causal_mask: Any, attention_mask: Any=None, head_mask: Any=None) -> object:
        device = query.device
        ...
        if self.scale_attn_weights:
            ...
            attn_weights = attn_weights / torch.full([], size_temp ** 0.5, dtype=attn_weights.dtype, device=attn_weights.device)
        if self.use_cache_quantization:
        ...
        if self.use_cache_quantization:
            ...
            if self.use_cache_kernel and self.cache_kernels is not None:
                ...
                self.cache_kernels.vecquant8matmul_batched_column_compression_faster_old(attn_weights.contiguous() if attn_weights.dtype == torch.float16 else attn_weights.to(torch.float16).contiguous(), qv.contiguous(), attn_output, qv_scale.contiguous() if qv_scale.dtype == torch.float16 else qv_scale.to(torch.float16).contiguous(), qv_zero.contiguous() if qv_zero.dtype == torch.float16 else qv_zero.to(torch.float16).contiguous())
    ...
    def _split_heads(self: Any, tensor: Any, num_heads: Any, attn_head_size: Any) -> object:
        new_shape = tensor.size()[:-1] + (num_heads, attn_head_size)
        tensor = tensor.view(new_shape)
        return tensor

    def _merge_heads(self: Any, tensor: Any, num_heads: Any, attn_head_size: Any) -> object:
        tensor = tensor.contiguous()
        new_shape = tensor.size()[:-2] + (num_heads * attn_head_size,)
        return tensor.view(new_shape)

    def forward(self: Any, hidden_states: Optional[Tuple[torch.FloatTensor]], rotary_pos_emb_list: Optional[List[List[torch.Tensor]]]=None, layer_past: Optional[Tuple[torch.Tensor]]=None, attention_mask: Optional[torch.FloatTensor]=None, head_mask: Optional[torch.FloatTensor]=None, encoder_hidden_states: Optional[torch.Tensor]=None, encoder_attention_mask: Optional[torch.FloatTensor]=None, output_attentions: Optional[bool]=False, use_cache: Optional[bool]=False) -> object:
        ...
        if self.use_flash_attn and flash_attn_unpadded_func is not None and (not self.is_fp32) and query.is_cuda:
            ...
            registered_causal_mask = torch.tril(torch.ones((key.size(1), key.size(1)), dtype=torch.bool, device=key.device)).view(1, 1, key.size(1), key.size(1))
            ...
            if not self.use_cache_quantization and SUPPORT_TORCH2:
                ...
                if attention_mask is not None:
                    attention_mask = attention_mask.expand(-1, -1, causal_mask.size(2), -1).masked_fill(~causal_mask, torch.finfo(query.dtype).min)
                else:
...
class QWenBlock(nn.Module):
    ...
    def forward(self: Any, hidden_states: Optional[Tuple[torch.FloatTensor]], rotary_pos_emb_list: Optional[List[List[torch.Tensor]]]=None, layer_past: Optional[Tuple[torch.Tensor]]=None, attention_mask: Optional[torch.FloatTensor]=None, head_mask: Optional[torch.FloatTensor]=None, encoder_hidden_states: Optional[torch.Tensor]=None, encoder_attention_mask: Optional[torch.FloatTensor]=None, use_cache: Optional[bool]=False, output_attentions: Optional[bool]=False) -> object:
        layernorm_output = self.ln_1(hidden_states)
        attn_outputs = self.attn(layernorm_output, rotary_pos_emb_list, layer_past=layer_past, attention_mask=attention_mask, head_mask=head_mask, use_cache=use_cache, output_attentions=output_attentions)
...
class QWenPreTrainedModel(PreTrainedModel):
    ...
    def __init__(self: Any, *inputs: Any, **kwargs: Any) -> object:
...
class QWenModel(QWenPreTrainedModel):
    ...
    def set_input_embeddings(self: Any, new_embeddings: Any) -> object:
        self.wte = new_embeddings

    def get_ntk_alpha(self: Any, true_seq_len: Any) -> object:
        context_value = math.log(true_seq_len / self.seq_length, 2) + 1
        ntk_alpha = 2 ** math.ceil(context_value) - 1
        ntk_alpha = max(ntk_alpha, 1)
        return ntk_alpha

    def forward(self: Any, input_ids: Optional[torch.LongTensor]=None, past_key_values: Optional[Tuple[Tuple[torch.Tensor]]]=None, attention_mask: Optional[torch.FloatTensor]=None, token_type_ids: Optional[torch.LongTensor]=None, position_ids: Optional[torch.LongTensor]=None, head_mask: Optional[torch.FloatTensor]=None, inputs_embeds: Optional[torch.FloatTensor]=None, encoder_hidden_states: Optional[torch.Tensor]=None, encoder_attention_mask: Optional[torch.FloatTensor]=None, use_cache: Optional[bool]=None, output_attentions: Optional[bool]=None, output_hidden_states: Optional[bool]=None, return_dict: Optional[bool]=None, audio_info: Dict=None) -> object:
        if audio_info is not None and past_key_values is None and torch.any(input_ids == self.config.audio['audio_start_id']):
        ...
        elif self.use_cache_quantization:
        ...
        if self.gradient_checkpointing and self.training:
            if use_cache:
                __jac_log_emit__(_gm_log_slot_0, ('`use_cache=True` is incompatible with gradient checkpointing. Setting `use_cache=False`...',), {})
                use_cache = False
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                outputs = torch.utils.checkpoint.checkpoint(create_custom_forward(block), hidden_states, rotary_pos_emb_list, None, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask)
            else:
                outputs = block(hidden_states, layer_past=layer_past, rotary_pos_emb_list=rotary_pos_emb_list, attention_mask=attention_mask, head_mask=head_mask[i], encoder_hidden_states=encoder_hidden_states, encoder_attention_mask=encoder_attention_mask, use_cache=use_cache, output_attentions=output_attentions)
...
class QWenLMHeadModel(QWenPreTrainedModel):
    ...
    def __init__(self: Any, config: Any) -> object:
        ...
        if autoset_precision:
            if SUPPORT_BF16:
                logger.warn('The model is automatically converting to bf16 for faster inference. "\n                    "If you want to disable the automatic precision, please manually add bf16/fp16/fp32=True to "AutoModelForCausalLM.from_pretrained".')
                config.bf16 = True
            elif SUPPORT_FP16:
                logger.warn('The model is automatically converting to fp16 for faster inference. "\n                    "If you want to disable the automatic precision, please manually add bf16/fp16/fp32=True to "AutoModelForCausalLM.from_pretrained".')
                config.fp16 = True
            else:
                config.fp32 = True
    ...
    @classmethod
    def from_pretrained(cls: Any, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args: Any, config: Any=None, cache_dir: Any=None, **kwargs: Any) -> object:
        if os.path.isdir(pretrained_model_name_or_path):
            ...
            hf_hub_download(repo_id=pretrained_model_name_or_path, filename='mel_filters.npz', token=kwargs.get('token', None), local_dir=os.path.dirname(__file__))
    ...
    def prepare_inputs_for_generation(self: Any, input_ids: Any, past_key_values: Any=None, inputs_embeds: Any=None, **kwargs: Any) -> object:
        ...
        model_inputs.update({'past_key_values': past_key_values, 'use_cache': kwargs.get('use_cache'), 'position_ids': position_ids, 'attention_mask': attention_mask, 'token_type_ids': token_type_ids, 'audio_info': audio_info})
        return model_inputs

    def forward(self: Any, input_ids: Optional[torch.LongTensor]=None, past_key_values: Optional[Tuple[Tuple[torch.Tensor]]]=None, attention_mask: Optional[torch.FloatTensor]=None, token_type_ids: Optional[torch.LongTensor]=None, position_ids: Optional[torch.LongTensor]=None, head_mask: Optional[torch.FloatTensor]=None, inputs_embeds: Optional[torch.FloatTensor]=None, encoder_hidden_states: Optional[torch.Tensor]=None, encoder_attention_mask: Optional[torch.FloatTensor]=None, labels: Optional[torch.LongTensor]=None, use_cache: Optional[bool]=None, output_attentions: Optional[bool]=None, output_hidden_states: Optional[bool]=None, return_dict: Optional[bool]=None, audio_info: Dict=None) -> Union[Tuple, CausalLMOutputWithPast]:
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        transformer_outputs = self.transformer(input_ids, past_key_values=past_key_values, attention_mask=attention_mask, token_type_ids=token_type_ids, position_ids=position_ids, head_mask=head_mask, inputs_embeds=inputs_embeds, encoder_hidden_states=encoder_hidden_states, encoder_attention_mask=encoder_attention_mask, use_cache=use_cache, output_attentions=output_attentions, output_hidden_states=output_hidden_states, return_dict=return_dict, audio_info=audio_info)
    ...
    def chat(self: Any, tokenizer: PreTrainedTokenizer, query: str, history: Optional[HistoryType], system: str='You are a helpful assistant.', append_history: bool=True, stream: Optional[bool]=_SENTINEL, stop_words_ids: Optional[List[List[int]]]=None, generation_config: Optional[GenerationConfig]=None, **kwargs: Any) -> Tuple[str, HistoryType]:
        ...
        outputs = self.generate(input_ids, stop_words_ids=stop_words_ids, return_dict_in_generate=False, generation_config=generation_config, **kwargs)
        response = decode_tokens(outputs[0], tokenizer, raw_text_len=len(raw_text), context_length=len(context_tokens), chat_format=generation_config.chat_format, verbose=False, errors='replace', audio_info=audio_info)
        history.append((query, response))
        return (response, history)

    def chat_stream(self: Any, tokenizer: PreTrainedTokenizer, query: str, history: Optional[HistoryType], system: str='You are a helpful assistant.', stop_words_ids: Optional[List[List[int]]]=None, logits_processor: Optional[LogitsProcessorList]=None, generation_config: Optional[GenerationConfig]=None, **kwargs: Any) -> Generator[str, Any, None]:
        generation_config = generation_config if generation_config is not None else self.generation_config
        assert generation_config.chat_format == 'chatml', _ERROR_BAD_CHAT_FORMAT
        if history is None:
            history = []
        ...
        if max_window_size is None:
            max_window_size = generation_config.max_window_size
        raw_text, context_tokens = make_context(tokenizer, query, history=history, system=system, max_window_size=max_window_size, chat_format=generation_config.chat_format)
        stop_words_ids.extend(get_stop_words_ids(generation_config.chat_format, tokenizer))
        if stop_words_ids is not None:
            stop_words_logits_processor = StopWordsLogitsProcessor(stop_words_ids=stop_words_ids, eos_token_id=generation_config.eos_token_id)
    ...
    def generate(self: Any, inputs: Optional[torch.Tensor]=None, generation_config: Optional[GenerationConfig]=None, logits_processor: Optional[LogitsProcessorList]=None, stopping_criteria: Optional[StoppingCriteriaList]=None, prefix_allowed_tokens_fn: Optional[Callable[[int, torch.Tensor], List[int]]]=None, synced_gpus: Optional[bool]=None, assistant_model: Optional['PreTrainedModel']=None, streamer: Optional['BaseStreamer']=None, **kwargs: Any) -> Union[GenerateOutput, torch.LongTensor]:
        ...
        if stop_words_ids is not None:
            ...
            if logits_processor is None:
                ...
                logits_processor.append(stop_words_logits_processor)
        return super().generate(inputs, generation_config=generation_config, logits_processor=logits_processor, stopping_criteria=stopping_criteria, prefix_allowed_tokens_fn=prefix_allowed_tokens_fn, synced_gpus=synced_gpus, assistant_model=assistant_model, streamer=streamer, **kwargs)

class RotaryEmbedding(torch.nn.Module):

    def __init__(self: Any, dim: Any, base: Any=10000) -> object:
        super().__init__()
        self.dim = dim
        self.base = base
    ...
    def update_rotary_pos_emb_cache(self: Any, max_seq_len: Any, offset: Any=0, ntk_alpha: Any=1.0) -> object:
    ...
    def forward(self: Any, max_seq_len: Any, offset: Any=0, ntk_alpha: Any=1.0) -> object:
        self.update_rotary_pos_emb_cache(max_seq_len, offset, ntk_alpha)
        cos, sin = self._rotary_pos_emb_cache
        return [cos[:, offset:offset + max_seq_len], sin[:, offset:offset + max_seq_len]]

def _rotate_half(x: Any) -> object:
    from einops import rearrange
    x = rearrange(x, '... (j d) -> ... j d', j=2)
    x1, x2 = x.unbind(dim=-2)
    return torch.cat((-x2, x1), dim=-1)

def apply_rotary_pos_emb(t: Any, freqs: Any) -> object:
    cos, sin = freqs
...
class RMSNorm(torch.nn.Module):
    ...
    def __init__(self: Any, dim: int, eps: float=1e-06) -> object:
        super().__init__()
        self.eps = eps
        self.weight = nn.Parameter(torch.ones(dim))

    def _norm(self: Any, x: Any) -> object:
