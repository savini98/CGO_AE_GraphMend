...
| `run_break_analysis.sh` | **One command for the break-elimination claim, end to end.** Checks the interpreter, the torch and transformers versions and the typeshed stubs, then runs the analysis with the right working directory and PYTHONPATH. Prints a table and exits non-zero if any row errors or changes its output. |
...
| `gpu/from_trace.py` | Re-derives cold, warm and launch numbers from a PyTorch profiler trace pair. **Needs no GPU and no model download.** Point it at traces produced by `artifact/claim2_latency.py`; no traces are shipped. |
